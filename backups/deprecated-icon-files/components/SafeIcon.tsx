'use client';

import React from 'react';
import { SafeIconProps, SIZE_CLASSES, IconSize } from '../types';
import cn from 'classnames';

/**
 * SafeIcon Component - Fallback for critical button icons
 * 
 * This component renders the most critical button icons directly, bypassing
 * the dynamic loading system for maximum reliability.
 */
export const SafeIcon: React.FC<SafeIconProps> = ({
  icon,
  className,
  solid = false,
  size = 'md',
  iconType = 'static',
  action = 'default',
  title
}) => {
  // Size class based on the size prop
  const sizeClass = SIZE_CLASSES[size as IconSize] || SIZE_CLASSES.md;

  // Helper for hover effects
  const getHoverClasses = () => {
    if (iconType !== 'button') return '';

    // For button icons, add transition effects
    const baseHoverClass = 'transition-colors duration-200';

    // Different classes based on the action type
    switch (action) {
      case 'delete':
        return `${baseHoverClass} group-hover:text-red-500`;
      case 'warning':
        return `${baseHoverClass} group-hover:text-yellow-500`;
      case 'success':
        return `${baseHoverClass} group-hover:text-green-500`;
      default:
        return `${baseHoverClass} group-hover:text-blue-500`;
    }
  };

  // Compute CSS classes for the icon
  const cssClasses = cn(
    'inline-block',
    sizeClass,
    getHoverClasses(),
    className
  );

  // For PenToSquare/Edit icon - simplified for reliability
  if (icon === 'faPenToSquare' || icon === 'faEdit') {
    return (
      <svg
        className={cssClasses}
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 512 512"
        fill="currentColor"
        aria-hidden={!title}
        role={title ? 'img' : 'presentation'}>
        {title && <title>{title}</title>}
        <path d="M471.6 21.7c-21.9-21.9-57.3-21.9-79.2 0L362.3 51.7l97.9 97.9 30.1-30.1c21.9-21.9 21.9-57.3 0-79.2L471.6 21.7zm-299.2 220c-6.1 6.1-10.8 13.6-13.5 21.9l-29.6 88.8c-2.9 8.6-.6 18.1 5.8 24.6s15.9 8.7 24.6 5.8l88.8-29.6c8.2-2.7 15.7-7.4 21.9-13.5L437.7 172.3 339.7 74.3 172.4 241.7zM96 64C43 64 0 107 0 160V416c0 53 43 96 96 96H352c53 0 96-43 96-96V320c0-17.7-14.3-32-32-32s-32 14.3-32 32v96c0 17.7-14.3 32-32 32H96c-17.7 0-32-14.3-32-32V160c0-17.7 14.3-32 32-32h96c17.7 0 32-14.3 32-32s-14.3-32-32-32H96z" />
      </svg>
    );
  }

  // For other critical icons, use a simple fallback
  return (
    <svg
      className={cssClasses}
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 512 512"
      fill="currentColor"
      aria-hidden={!title}
      role={title ? 'img' : 'presentation'}>
      {title && <title>{title}</title>}
      <path d="M256 512A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM369 209L241 337c-9.4 9.4-24.6 9.4-33.9 0l-64-64c-9.4-9.4-9.4-24.6 0-33.9s24.6-9.4 33.9 0l47 47L335 175c9.4-9.4 24.6-9.4 33.9 0s9.4 24.6 0 33.9z" />
    </svg>
  );
}; 