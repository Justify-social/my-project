...
export function getIconPath(iconName: string, style: string = 'light'): string {
  ...
  if (iconName.startsWith('fab')) {
    ...
    // App navigation icons - special handling for app icons
    // Important: We need to handle both PascalCase and kebab-case formats
    
    // Extract the actual icon name after the 'app' prefix
    const iconNameWithoutPrefix = iconName.slice(3); // Remove 'app' prefix
    
    // Return the path with the exact original casing preserved
    // This ensures the casing matches the actual file names
    // Example: appHome -> /icons/app/Home.svg
    // Example: appBrand_Health -> /icons/app/Brand_Health.svg
    return `/icons/app/${iconNameWithoutPrefix}.svg`;
  }
...
function isAppIcon(iconName: string): boolean {
  // Check if the icon name starts with 'app'
  return iconName.startsWith('app');
} 