...
// Note: We'll need the pro package for light icons
// This script checks for issues but doesn't require the actual package
// to be loaded if it's not available

// Get the directory of the current module
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

// Path to the light icon registry JSON file
const ICON_REGISTRY_PATH = path.resolve(process.cwd(), 'public/static/light-icon-registry.json');
  ...
  try {
    ...
    for (const icon of iconRegistry.icons) {
      ...
      if (!faVersion) {
        ...
        const suggestedVersion = `fal fa-${name.toLowerCase().replace(/\s+/g, '-')}`;
        missingFaVersion.push({ id, name, suggestedVersion });
      }
      
      if (!iconPath) {
        missingPaths.push({ id, name });
      } else if (!iconPath.startsWith('/icons/light/')) {
        // Path format issue
        const correctPath = `/icons/light/${iconPath}`;
        incorrectPathFormat.push({ id, name, currentPath: iconPath, correctPath });
      }
      
      // If no issues, it's a valid icon
      if (faVersion && iconPath && iconPath.startsWith('/icons/light/')) {
    ...
    console.log(`   ❌ Issues detected: ${missingFaVersion.length + incorrectPathFormat.length + missingPaths.length}`);
    ...
    if (incorrectPathFormat.length > 0) {
      ...
      incorrectPathFormat.forEach(({ id, name, currentPath, correctPath }) => {
        console.log(`   - ${id} (${name}): should use "${correctPath}" instead of "${currentPath}"`);
      });
    }
    ...
    if (missingFaVersion.length > 0 || incorrectPathFormat.length > 0 || missingPaths.length > 0) {
      ...
      console.log("   2. Correct path formats to include '/icons/light/' prefix");
