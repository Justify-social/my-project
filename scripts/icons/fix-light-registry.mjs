...
const iconFaVersionFixes = {
  'faCircleCheckLight': 'fal fa-circle-check',
  'faCircleInfoLight': 'fal fa-circle-info',
  'faCircleQuestionLight': 'fal fa-circle-question',
  'faClockRotateLeftLight': 'fal fa-clock-rotate-left',
  'faHouseLight': 'fal fa-house',
  'faPenToSquareLight': 'fal fa-pen-to-square',
  'faTrashCanLight': 'fal fa-trash-can',
  'faTriangleExclamationLight': 'fal fa-triangle-exclamation',
  'faXmarkLight': 'fal fa-xmark'
};

// Main function to fix the registry
function fixLightIconRegistry() {
  try {
    ...
    iconRegistry.icons = iconRegistry.icons.map(icon => {
      ...
      if (icon.path && !icon.path.startsWith('/icons/light/')) {
        updatedIcon.path = `/icons/light/${icon.path}`;
    ...
    console.log(`\n🎉 Light icon registry is now ready for secure locking!`);
