 ...
 * 
 * Usage:
 *   node scripts/icons/robust-icon-downloader.js [options]
 * 
 * Options:
...
const ICON_REGISTRY_PATH = path.join(__dirname, '..', '..', '..', 'src', 'components', 'ui', 'atoms', 'icons', 'mapping', 'icon-registry.json');
const ICON_URL_MAP_PATH = path.join(__dirname, '..', '..', '..', 'src', 'components', 'ui', 'atoms', 'icons', 'mapping', 'icon-url-map.json');
...
const REQUIRED_ICONS = {
  'light': [
    ...
    'pen-to-square', 'atom', 'dna', 'bacteria'
  ],
  'solid': [
    'bell', 'circle-check', 'check', 'xmark', 'circle-info', 'gear',
    'house', 'atom', 'dna', 'bacteria'
  ],
  'brands': [
...
// Add an icon to the registry file
function addIconToRegistry(registry, iconName, style, filename) {
  const prefix = ICON_PREFIXES[style];
  const path = calculateIconPath(style, filename);
  const baseName = getBaseName(iconName);
  
  // Create the registry entry
  registry[iconName] = {
    prefix,
    name: style === 'app' || style === 'kpis' ? baseName : filename.replace('.svg', ''),
    fileName: filename,
    path
  };
...
function updateRegistryFiles(iconName, style, filename) {
  ...
  // If we have a light icon, also add the Light suffix variant
  if (style === 'light') {
    const lightVariant = `${iconName}Light`;
    addIconToRegistry(registry, lightVariant, style, filename);
    addIconToUrlMap(urlMap, lightVariant, style, filename);
  }
  
  // Save the registry files
  saveRegistryFile(ICON_REGISTRY_PATH, registry);
  saveRegistryFile(ICON_URL_MAP_PATH, urlMap);
}
