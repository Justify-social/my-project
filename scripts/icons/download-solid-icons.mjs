...
const ICON_REGISTRY_PATH = path.resolve(process.cwd(), 'public/static/new-solid-icon-registry.json');
...
function processIcon(iconData) {
  ...
  try {
    ...
    // Handle solid icons from the @fortawesome/pro-solid-svg-icons package
    if (style === 'fas') {
      ...
      // Get the icon definition from the solid package
      iconDefinition = proSolidIcons[faIconName];
      
      if (!iconDefinition) {
        console.warn(`⚠️ Icon not found in solid icons: ${faIconName}. Available similar icons:`);
        const similarIcons = Object.keys(proSolidIcons)
      ...
      console.warn(`⚠️ Non-solid icons not supported in this version: ${style}`);
      return false;
    }
