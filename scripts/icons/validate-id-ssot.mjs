      ...
      for (let i = 0; i < iconCount; i++) {
        ...
        if (icon.id !== icon.name && icon.name && typeof icon.name === 'string') {
          if (icon.name.startsWith('fa') && !icon.name.includes(' ')) {
            issues.push(
              `⚠️ Icon '${icon.id}' in ${filePath} has a name '${icon.name}' that could be confused with an ID`
            );
          }
        }
        ...
        if (icon.category === 'kpis' && !icon.id.startsWith('kpis')) {
          issues.push(
            `⚠️ Icon '${icon.id}' in KPIs registry doesn't follow naming convention (should start with 'kpis')`
          );
        }
        ...
        if (!icon.path.startsWith(expectedPathPrefix)) {
          issues.push(
            `⚠️ Icon '${icon.id}' in ${filePath} has path '${icon.path}' that doesn't match its category '${icon.category}'`
          );
        }
