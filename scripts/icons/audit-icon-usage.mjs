...
function analyzeIconUsage(iconName, props) {
  ...
  if (faVersionToIdMap.has(iconName)) {
    ...
    return {
      ...
      fixedProps: props.replace(/name\s*=\s*["']([^"']+)["']/, `name="${correctId}"`),
    };
  }
...
function findSimilarIconIds(name) {
  ...
  for (const id of allIconIds) {
    if (id.toLowerCase().includes(lowercaseName) || lowercaseName.includes(id.toLowerCase())) {
      similarIds.push(id);
    }
  }
...
async function main() {
  ...
  if (SHOULD_FIX) {
    console.log(
      chalk.yellow('Running in FIX mode - will attempt to correct issues automatically\n')
    );
  } else {
