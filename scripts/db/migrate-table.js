 ...
 * 
 * This script helps migrate usages of the old table component to the new modular table component.
 * It searches for import statements that reference the old table.tsx file and replaces them
 * with the new modular import path.
 * 
 * Usage:
 * node migrate-table.js [--dry-run]
 */
...
// Helper functions
function findFilesImportingOldTable() {
  console.log('Searching for files importing old table component...');
...
function processFile(filePath) {
  ...
  for (const pattern of SEARCH_PATTERNS) {
    ...
    content = content.replace(namedImportRegex, (match, imports) => {
      modified = true;
      return `import { ${imports} } from "@/components/ui/table";`;
    });
    
    // Default import pattern: import Table from "pattern";
    ...
    content = content.replace(defaultImportRegex, (match, importName) => {
      modified = true;
      return `import ${importName} from "@/components/ui/table";`;
    });
  }
...
// Main execution
function main() {
  console.log('Table Component Migration Script');
  console.log('==============================');
  console.log(`Mode: ${DRY_RUN ? 'Dry Run (no changes will be made)' : 'Live Run'}`);
  
  // Find files that import the old table component
  const filesToProcess = findFilesImportingOldTable();
  
  if (filesToProcess.length === 0) {
    console.log('No files found importing the old table component.');
    return;
  }
