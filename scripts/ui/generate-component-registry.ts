...
function extractMetadata(filePath: string): ComponentMetadata | null {
  try {
    ...
    console.log(
      `DEBUG: Parsed Doctrine for ${path.basename(filePath)}:`,
...
console.log(
  'DEBUG: Raw components found before filtering:',
  JSON.stringify(allComponents, null, 2)
); // Log all found components before filtering

console.log(`Found ${allComponents.length} potential component files.`);

// Filter components by allowed categories
const filteredComponents = allComponents.filter(
  comp => comp.category && ALLOWED_CATEGORIES.includes(comp.category)
);

console.log(
  `Filtered down to ${filteredComponents.length} components in categories: ${ALLOWED_CATEGORIES.join(', ')}`
);
