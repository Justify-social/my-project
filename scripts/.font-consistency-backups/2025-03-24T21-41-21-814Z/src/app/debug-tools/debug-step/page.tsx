...
export default function DebugStep() {
      ...
      <div className="bg-[var(--background-color)] rounded-lg border border-[var(--divider-color)] p-6 shadow-sm">
        <div className="flex justify-between mb-4">
          ...
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${
            stepData.status === 'complete' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
          }`}>
            {stepData.status}
          </span>
        </div>
        ...
        <div className="space-y-4">
          {Object.entries(stepData.fields).map(([fieldName, validation], index) => (
            <div key={index} className="border-b border-[var(--divider-color)] pb-3">
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-medium text-[var(--primary-color)]">{fieldName}</p>
                  <span className={`text-sm ${getStatusColor(validation.status)}`}>
                    {validation.status}
                    {validation.error && <span className="ml-2 text-red-500">Error: {validation.error}</span>}
                  </span>
                </div>
  ...
  return (
    <div className="container mx-auto p-6 max-w-5xl">
      <h1 className="text-3xl font-bold mb-6 text-[var(--primary-color)]">Campaign Wizard Step Debug</h1>
      ...
      <div className="bg-[var(--background-color)] rounded-lg border border-[var(--divider-color)] p-6 mb-6 shadow-sm">
        <div className="flex gap-4 mb-6">
          ...
          <select
            ...
            className="p-2 border border-[var(--divider-color)] rounded-md focus:ring-[var(--accent-color)] focus:border-[var(--accent-color)]"
          >
        ...
        <button
          ...
          className="px-4 py-2 bg-gray-100 text-[var(--primary-color)] rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-[var(--accent-color)] focus:ring-offset-2"
        >
          Go to Step {step}
        </button>
        
        {error && (
          <div className="mb-4 mt-4 p-3 bg-red-50 text-red-600 border border-red-200 rounded-md">
            {error}
          </div>
        )}
      </div>
