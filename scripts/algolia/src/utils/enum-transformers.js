...
exports.EnumTransformers = {
    ...
    invitationStatusToBackend: function (value) {
        debugLog("[EnumTransformer] invitationStatusToBackend: ".concat(value, " \u2192 ").concat(value));
        return value; // Both use UPPERCASE format
    },
    invitationStatusFromBackend: function (value) {
        debugLog("[EnumTransformer] invitationStatusFromBackend: ".concat(value, " \u2192 ").concat(value));
    ...
    transformObjectToBackend: function (obj) {
        ...
        if (Array.isArray(obj)) {
            return obj.map(function (item) { return _this.transformObjectToBackend(item); });
        }
        ...
        for (var _i = 0, _a = Object.entries(obj); _i < _a.length; _i++) {
            ...
            if (key === 'kpi' || key === 'primaryKPI') {
                ...
                result[key] = value.map(function (kpi) { return _this.kpiToBackend(kpi); });
            }
                ...
                result[key] = value.map(function (feature) { return _this.featureToBackend(feature); });
            }
            else if (key === 'role') {
                result[key] = this.teamRoleToBackend(value);
            }
            else if (key === 'status') {
                result[key] = this.invitationStatusToBackend(value);
            }
    ...
    transformObjectFromBackend: function (obj) {
        ...
        if (!obj)
            return obj;
        if (Array.isArray(obj)) {
            return obj.map(function (item) { return _this.transformObjectFromBackend(item); });
        }
        if (typeof obj !== 'object')
            return obj;
        var result = {};
        for (var _i = 0, _a = Object.entries(obj); _i < _a.length; _i++) {
            ...
            if (key === 'kpi' || key === 'primaryKPI') {
                ...
                result[key] = value.map(function (kpi) { return _this.kpiFromBackend(kpi); });
            }
            else if (key === 'platform') {
                result[key] = this.platformFromBackend(value);
            }
            else if (key === 'currency') {
                result[key] = this.currencyFromBackend(value);
            }
            else if (key === 'position') {
                result[key] = this.positionFromBackend(value);
            }
            else if (key === 'features' && Array.isArray(value)) {
                result[key] = value.map(function (feature) { return _this.featureFromBackend(feature); });
            }
            else if (key === 'role') {
                result[key] = this.teamRoleFromBackend(value);
            }
            else if (key === 'status') {
                result[key] = this.invitationStatusFromBackend(value);
            }
            else if (key === 'userRole') {
                result[key] = this.userRoleFromBackend(value);
            }
            else if (value instanceof Date) { // Explicitly handle Date objects
