    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Last attempt with bare minimum config
          try {
        ...
        traverse(ast, {
          ...
          VariableDeclaration(path) {
            ...
            declarations.forEach(declaration => {
              ...
              if (!isExported) {
                // Look for export statements referring to this variable
                // This is simplistic; consider using scope analysis for accuracy
                const fileAst = path.hub && path.hub.file ? path.hub.file.ast : ast;
                if (fileAst) {
                  try {
                    traverse.simple(fileAst, {
                      ExportNamedDeclaration(node) {
                        if (node.specifiers) {
                          for (const specifier of node.specifiers) {
                            if (specifier.local && specifier.local.name === name) {
                              isExported = true;
                              break;
                            }
                          }
                      ...
                      ExportDefaultDeclaration(node) {
                        if (t.isIdentifier(node.declaration) && node.declaration.name === name) {
                          isExported = true;
                        }
                      }
                    });
                  } catch (traverseError) {
                    logger.warn(`Error during simple traversal for ${relativeFilePath}: ${traverseError.message}`);
                    // If traverse fails, assume it might be exported to avoid missing components
                    isExported = true;
                  }
                } else {
                  // If AST or hub is not available, assume it might be exported
                  logger.warn(`Couldn't access AST for ${relativeFilePath}, assuming component might be exported`);
                  isExported = true;
                }
              }
              ...
              if (isExported) {
                hasExportedComponents = true;
                logger.verbose(`Found exported variable component: ${name} in ${relativeFilePath}`);
                ...
                components.push({
                  ...
                  lastUpdated: new Date().toISOString(),
                  exports: [name],
                  props: [], // Simplified for variable declarations
                  description: `UI ${name} component`
                });
              }
            });
