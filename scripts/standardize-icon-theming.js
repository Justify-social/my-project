...
// Add this script to the migrate-all-icons.js script if not already present
const migrateScriptPath = 'scripts/migrate-all-icons.js';

try {
  if (fs.existsSync(migrateScriptPath)) {
    let migrateContent = fs.readFileSync(migrateScriptPath, 'utf8');
    
    if (!migrateContent.includes('standardize-icon-theming.js')) {
      ...
      // Find the location just before the final console.log
      const finalLogIndex = migrateContent.lastIndexOf('console.log("✅');
      
      if (finalLogIndex !== -1) {
        migrateContent = 
          migrateContent.substring(0, finalLogIndex) + 
          newStep + 
          '\n\n' + 
          migrateContent.substring(finalLogIndex);
        
        fs.writeFileSync(migrateScriptPath, migrateContent, 'utf8');
        console.log(`${colors.green}✅ Added standardize-icon-theming step to ${migrateScriptPath}${colors.reset}`);
      } else {
        console.log(`${colors.yellow}⚠️ Could not find the right location to add the new step in ${migrateScriptPath}${colors.reset}`);
      }
    } else {
      console.log(`${colors.green}✅ standardize-icon-theming step already exists in ${migrateScriptPath}${colors.reset}`);
    }
  } else {
    console.log(`${colors.yellow}⚠️ Could not find the migration script at ${migrateScriptPath}${colors.reset}`);
  }
} catch (err) {
  console.error(`Error updating the migration script:`, err);
} 