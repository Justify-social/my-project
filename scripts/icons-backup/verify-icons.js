...
async function verifyIcons() {
  ...
  try {
    ...
    if (fs.existsSync(lightDir) && fs.existsSync(solidDir)) {
      ...
      try {
        const output = execSync('node scripts/icon-management/audit-icons.js --verbose', { encoding: 'utf8' });
        if (output.includes('duplicate light/solid icon pairs')) {
          ...
          console.log(`${colors.yellow}Run 'node scripts/icon-management/audit-icons.js --fix-duplicates' to fix this issue.${colors.reset}`);
        } else {
