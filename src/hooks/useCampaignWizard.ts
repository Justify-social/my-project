...
export type CampaignFormData = OverviewFormData & ObjectivesFormData & AudienceFormData & AssetFormData;
...
export function useCampaignWizard({
   ...
   */
  const loadStepData = useCallback(async (step: WizardStep) => {
    ...
    try {
      dbLogger.info(
        DbOperation.FETCH,
        `Loading data for campaign ${campaignId} step ${step}`,
        { campaignId, step }
      );
      ...
      const errorMessage = err instanceof Error ? err.message : 'An error occurred loading step data';
   ...
   */
  const saveStepData = useCallback(async (step: WizardStep, isAutoSave = false) => {
    if (!campaignId) return;
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'An error occurred saving step data';
      setError(errorMessage);
  ...
  const submitCampaign = useCallback(async () => {
    ...
    if (saved) {
      ...
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'An error occurred submitting the campaign';
        setError(errorMessage);
