...
export async function getUpcomingEvents(userId: string): Promise<CalendarEvent[]> {
    ...
    try {
        ...
        return (campaignsWithInfluencers as CampaignWithInfluencers[]).map((campaign): CalendarEvent => {
            const influencers: Influencer[] = campaign.Influencer || [];
            const platforms = [...new Set(influencers
            ...
            try {
                ...
                if (typeof campaign.budget === 'object' && campaign.budget !== null && 'total' in campaign.budget && typeof campaign.budget.total === 'number') {
                    ...
                    if (typeof parsed === 'number') {
                        budgetAmount = parsed;
                    } else if (typeof parsed === 'object' && parsed !== null && 'total' in parsed && typeof parsed.total === 'number') {
                        budgetAmount = parsed.total;
                    }
                }
            } catch (e) { console.error("Error parsing budget JSON:", e); }

            const isAllDay = !campaign.endDate || isSameDay(campaign.startDate, campaign.endDate || campaign.startDate);
...
export async function getUpcomingCampaigns(userId: string): Promise<CampaignData[]> {
    ...
    try {
        ...
        return (campaignsWithInfluencers as CampaignWithPrimaryInfluencer[]).map((campaign): CampaignData => {
            // Type assertion might be needed if TS can't infer from include+select+take
            const primaryInfluencer: { handle: string; platform: Platform } | undefined = campaign.Influencer?.[0];
