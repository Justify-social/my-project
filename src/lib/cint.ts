...
export class CintApiService {
    ...
    constructor(clientId?: string, clientSecret?: string, accountId?: string, s2sApiKey?: string) {
        ...
        if (!this.mockEnabled && (!this.clientId || !this.clientSecret || !this.accountId || !this.s2sApiKey)) {
            const message = 'Cint API credentials or Account ID are not configured in environment variables. Live mode will fail.';
    ...
    private async _fetchWithRetry(url: string, options: RequestInit, retries = 3, attempt = 1): Promise<Response> {
        try {
            logger.debug(`[CintAPI] Fetching (Attempt ${attempt}/${retries}): ${options.method || 'GET'} ${url}`);
            ...
            if (!response.ok) {
                ...
                if ((response.status === 429 || response.status >= 500) && attempt < retries) {
                    ...
                    logger.warn(`[CintAPI] Request failed with status ${response.status}. Retrying in ${waitTime}ms...`, { url, attempt });
                ...
                } catch (e) { errorBody = { message: await response.text() || response.statusText }; }
                logger.error('[CintAPI] Request failed after all retries or with client error', {
                    url, status: response.status, attempt, errorBody
                });
                const err = new Error(`Cint API Error: ${response.status} ${response.statusText} - ${errorBody?.error?.message || errorBody?.message}`);
            ...
            if (attempt < retries && !(error instanceof Error && (error as any).status && (error as any).status < 500 && (error as any).status !== 429)) {
                const waitTime = (2 ** attempt) * 1000;
                logger.warn(`[CintAPI] Network/other error for ${url}. Retrying in ${waitTime}ms...`, { url, attempt, error: error.message });
                await delay(waitTime);
                return this._fetchWithRetry(url, options, retries, attempt + 1);
            }
            logger.error('[CintAPI] Fetch failed after all retries or with non-retryable error', { url, error: error.message });
    ...
    async authenticate(): Promise<CintOAuthToken> {
        ...
        try {
            const response = await this._fetchWithRetry(CINT_AUTH_URL, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(requestBody),
            }, 3); // Auth retries might be less aggressive or not needed.
            ...
            logger.error('[CintApiService] Cint authentication failed.', { error: error.message, status: error.status, body: error.errorBody });
            this.authToken = null;
    ...
    private async getAuthHeaders(): Promise<{ Authorization: string; 'Cint-API-Version': string; 'Content-Type': string; }> {
    ...
    async createCintProject(studyName: string, projectManagerId: string): Promise<CintProject> {
        if (this.mockEnabled) {
            logger.info('[CintApiService] Creating Cint Project (mock)', { accountId: this.accountId, studyName });
        ...
        logger.info('[CintApiService] Creating Cint Project (live)', { accountId: this.accountId, projectInput });
        try {
            ...
            logger.info('[CintApiService] Cint Project created successfully (live)', { projectId: projectData.id });
            return projectData;
        } catch (error: any) {
            logger.error('[CintApiService] Failed to create Cint Project (live)', { error: error.message, studyName });
    ...
    async createCintTargetGroup(
        ...
        if (this.mockEnabled) {
            logger.info('[CintApiService] Creating Cint Target Group (mock)', { cintProjectId, studyId: study.id });
            const mockTgId = `tg_mock_${Date.now()}`;
            const mockTgInput: CintTargetGroupInput = {
                name: `${study.name} - Target Group`.substring(0, 255),
                business_unit_id: businessUnitId,
                project_manager_id: projectManagerId,
                study_type_code: 'adhoc', industry_code: 'other', locale: 'eng_us', collects_pii: false,
        ...
        logger.info('[CintApiService] Creating Cint Target Group (live)', { accountId: this.accountId, cintProjectId, name: targetGroupInput.name });
        try {
            const response = await this._fetchWithRetry(
            ...
            logger.info('[CintApiService] Cint Target Group created successfully (live)', { targetGroupId: tgData.id });
            return tgData; // Should include id and status: 'draft' from Cint
        } catch (error: any) {
            logger.error('[CintApiService] Failed to create Cint Target Group (live)', { error: error.message, cintProjectId, studyId: study.id });
            throw error;
        }
    }
    ...
    async launchCintTargetGroup(
        ...
        if (this.mockEnabled) {
            logger.info('[CintApiService] Launching Cint Target Group (mock)', { cintProjectId, cintTargetGroupId });
        ...
        logger.info('[CintApiService] Launching Cint Target Group (live)', { accountId: this.accountId, cintProjectId, cintTargetGroupId, requestBody });
        try {
            const response = await this._fetchWithRetry(
            ...
            if (locationHeader) {
                ...
                } catch (e) { /* Ignore if body is not JSON or job_id not present */ }
            }

            logger.info('[CintApiService] Cint Target Group launch initiated/completed (live)', { targetGroupId: cintTargetGroupId, jobId, status: response.status });
            return { job_id: jobId };

        } catch (error: any) {
            logger.error('[CintApiService] Failed to launch Cint Target Group (live)', { error: error.message, cintTargetGroupId });
            throw error;
        }
    }
    ...
    async getTargetGroupOverview(
        ...
        if (this.mockEnabled) {
            logger.info('[CintApiService] Getting Target Group Overview (mock)', { cintProjectId, cintTargetGroupId });
            return {
                id: cintTargetGroupId,
                name: 'Mock Target Group Overview',
                status: 'live',
                statistics: {
                    filling_goal: 1000, current_completes: Math.floor(Math.random() * 500), current_prescreens: Math.floor(Math.random() * 1000),
                    incidence_rate_median: 0.5, length_of_interview_median_seconds: 600,
                    conversion_rate_average: 0.75, drop_off_rate_average: 0.1,
                },
            };
        }
        const headers = await this.getAuthHeaders();
        logger.info('[CintApiService] Getting Target Group Overview (live)', { accountId: this.accountId, cintProjectId, cintTargetGroupId });
        try {
            const response = await this._fetchWithRetry(
            ...
            logger.info('[CintApiService] Successfully fetched Target Group Overview (live)', { targetGroupId: cintTargetGroupId });
            return overviewData;
        } catch (error: any) {
            logger.error('[CintApiService] Failed to fetch Target Group Overview (live)', { error: error.message, cintTargetGroupId });
            throw error;
        }
    }
    ...
    async s2sValidateRespondent(respondentId: string): Promise<CintS2SValidateResponse> {
        ...
        if (!s2sKey && !this.mockEnabled) throw new Error('Cint S2S API Key not configured for live mode.');

        if (this.mockEnabled) {
            logger.info('[CintApiService] S2S Validate Respondent (mock)', { respondentId });
            return {
                id: respondentId, status: 1,
        ...
        try {
            ...
            logger.info('[CintApiService] S2S Respondent Validation successful (live)', { respondentId, status: validationData.status });
            return validationData;
        } catch (error: any) {
            logger.error('[CintApiService] S2S Respondent Validation failed (live)', { error: error.message, respondentId });
            throw error;
        }
    }
     ...
     */
    async s2sUpdateRespondentStatus(statusBody: CintS2SUpdateStatusBody): Promise<{ success: boolean; message?: string }> {
        const s2sKey = this.s2sApiKey;
        if (!s2sKey && !this.mockEnabled) throw new Error('Cint S2S API Key not configured for live mode.');
        ...
        try {
            ...
            logger.info('[CintApiService] S2S Respondent Status Update successful (live)', { respondentId: statusBody.id, newStatus: statusBody.status });
            return { success: true, message: await response.text() || 'Status updated' };
        } catch (error: any) {
            logger.error('[CintApiService] S2S Respondent Status Update failed (live)', { error: error.message, statusBody });
            throw error;
        }
    }
