...
import {
    ...
    NotFoundError,
    BadRequestError
} from './errors'; // Assuming errors.ts is in the same lib directory
...
export const handleApiError = (error: any, request?: NextRequest): NextResponse => {
    ...
    if (error instanceof UnauthenticatedError) {
        ...
        logger.warn('API Validation Error (Zod)', { errors: error.format(), ...requestInfo, statusCode });
        ...
        logger.error('API Unknown Error: Non-Error thrown', { errorDetails: error, ...requestInfo, statusCode });
        errorMessage = "An unexpected error occurred.";
    }

    return NextResponse.json({ error: errorMessage, details: error instanceof z.ZodError ? error.format() : undefined }, { status: statusCode });
}; 