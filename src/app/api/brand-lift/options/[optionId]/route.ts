...
async function verifyOptionAccess(optionId: string, orgId: string, userId: string) {
    ...
    const allowedStatuses = [Prisma.BrandLiftStudyStatus.DRAFT, Prisma.BrandLiftStudyStatus.PENDING_APPROVAL];
    if (!(allowedStatuses as Prisma.BrandLiftStudyStatus[]).includes(currentStudyStatus)) {
        throw new ForbiddenError(`Options cannot be modified when study status is ${currentStudyStatus}.`);
    }
    return option;
}

// PUT handler - REMOVING CONTEXT PARAMETER FOR DIAGNOSTICS
export async function PUT(req: NextRequest) { // Removed context parameter
    try {
        ...
        if (!validation.success) {
            logger.warn('Invalid option update data', { optionId: '?', errors: validation.error.flatten().fieldErrors, orgId });
...
export async function DELETE(req: NextRequest) { // Removed context parameter
    try {
        const { userId, orgId } = await auth();
        if (!userId || !orgId) throw new UnauthenticatedError('Unauthorized');
        ...
        return NextResponse.json({ message: 'Option deleted successfully (Placeholder - ID missing)' }, { status: 200 });
