...
const reorderOptionsPayloadSchema = z.array(reorderOptionItemSchema).min(1, "At least one option required for reorder.");

// Helper to verify question access and modifiable status of its parent study
async function verifyQuestionAccessForOptionReorder(questionId: string, orgId: string): Promise<SurveyQuestion & { study: { status: BrandLiftStudyStatus } }> {
    ...
    if (question.study.organizationId !== orgId) throw new ForbiddenError('Access denied to this question\'s study.');
    ...
    if (currentStudyStatus !== BrandLiftStudyStatus.DRAFT && currentStudyStatus !== BrandLiftStudyStatus.PENDING_APPROVAL) {
        throw new ForbiddenError(`Options cannot be reordered when study status is ${currentStudyStatus}.`);
    }
    return question as SurveyQuestion & { study: { status: BrandLiftStudyStatus } }; // Cast for type safety
}

export const PATCH = async (req: NextRequest, { params: paramsPromise }: { params: Promise<{ questionId: string }> }) => {
    try {
        ...
        if (!validation.success) {
            logger.warn('Invalid option reorder data', { questionId, errors: validation.error.flatten().fieldErrors, orgId });
        ...
        if (optionsInQuestion.length !== updates.length) {
            logger.error('Mismatch in option IDs provided for reorder', {
                questionId, orgId, providedIds: optionIdsToUpdate, foundIds: optionsInQuestion.map((o: { id: string }) => o.id)
            });
            throw new BadRequestError('Invalid option ID(s) for reorder. All options must belong to the specified question.');
        }
        ...
        logger.info('Successfully reordered options for question', { questionId, orgId, count: updates.length });
