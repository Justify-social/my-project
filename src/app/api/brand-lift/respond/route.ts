...
const cintS2SResponseSchema = z.object({
    ...
    answers: z.array(z.object({ // Highly simplified answer structure - Cint's actual format might be different
...
export const POST = async (req: NextRequest) => {
    try {
        ...
        if (!validation.success) {
            logger.warn('Invalid Cint webhook payload', { errors: validation.error.flatten().fieldErrors, receivedBody: body });
            return NextResponse.json({ error: 'Invalid payload', details: validation.error.flatten() }, { status: 400 });
        }

        const { id: cintRespondentId, status: cintStatus, custom_vars, answers: rawAnswers, demographics: rawDemographics } = validation.data;
        ...
        if (!studyId || !z.string().cuid().safeParse(studyId).success) {
            logger.error('Could not determine internal studyId from Cint webhook', { cintRespondentId, custom_vars });
        ...
        logger.info('SurveyResponse record created from Cint webhook', { surveyResponseId: surveyResponse.id, studyId, cintRespondentId });
        return NextResponse.json({ message: 'Webhook received successfully.', surveyResponseId: surveyResponse.id }, { status: 201 });
