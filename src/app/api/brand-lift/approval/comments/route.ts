...
import { Prisma, BrandLiftStudyStatus, SurveyApprovalCommentStatus, SurveyOverallApprovalStatus } from '@prisma/client';
...
const createCommentSchema = z.object({
  ...
  text: z.string().min(1, { message: 'Comment text cannot be empty.' }).max(2000, { message: 'Comment text too long.' }),
});
...
async function verifyStudyAccessForComments(studyId: string, orgId: string) {
  ...
  if (currentStatus === BrandLiftStudyStatus.ARCHIVED || currentStatus === BrandLiftStudyStatus.DRAFT) {
    throw new ForbiddenError(`Comments are not allowed when study status is ${currentStatus}. Study must be in a review phase.`);
  }
  return study;
}
...
export const POST = async (req: NextRequest) => {
  try {
    ...
    const approvalStatus = await db.surveyApprovalStatus.upsert({
      ...
      select: { id: true, study: { select: { name: true, campaign: { select: { userId: true } } } } } // Select needed for notification
    });

    const body = await req.json();
    const validation = createCommentSchema.safeParse(body);
    if (!validation.success) {
      logger.warn('Invalid comment creation data', { studyId, errors: validation.error.flatten().fieldErrors, orgId });
    ...
    if (questionId) {
      const question = await db.surveyQuestion.findUnique({ where: { id: questionId }, select: { text: true } });
    ...
    try {
      const commenterDetails = await db.user.findUnique({ where: { id: userId }, select: { email: true, name: true } });
      ...
      if (studyOwnerId) {
        studyOwnerDetails = await db.user.findUnique({ where: { id: studyOwnerId }, select: { id: true, email: true, name: true } });
      }
      ...
      if (commenterDetails?.email && studyOwnerDetails?.email && studyOwnerDetails.id !== userId) {
        ...
        await notificationServiceInstance.sendNewCommentEmail(
          ...
          { commentText: newComment.text, commentAuthorName: commenterDetails.name ?? commenterDetails.email, questionText: questionTextForNotification }
        );
        logger.info('New comment notification sent', { studyId, commentId: newComment.id });
      } else {
        logger.warn('Could not send new comment notification due to missing details or self-commenting.', { studyId, commenterId: userId, ownerId: studyOwnerId });
      }
    } catch (emailError: any) {
      logger.error('Failed to send new comment notification', { studyId, commentId: newComment.id, error: emailError.message });
    }
...
export const GET = async (req: NextRequest) => {
  try {
    ...
    if (!validation.success) {
      logger.warn('Invalid query params for fetching comments', { errors: validation.error.flatten().fieldErrors, orgId });
      throw validation.error;
    }
