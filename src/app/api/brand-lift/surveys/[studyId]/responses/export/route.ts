...
async function verifyStudyForExport(studyId: string, orgId: string) {
  ...
  if (!allowedStatuses.includes(study.status as BrandLiftStudyStatus)) {
    throw new ForbiddenError(
      `Response data cannot be exported when study status is ${study.status}.`
    );
  }
  return study;
}
...
export const GET = async (
  ...
  try {
    ...
    if (responses.length === 0) {
      ...
      return new NextResponse(emptyCsv, {
        ...
        headers: {
          ...
          'Content-Disposition': `attachment; filename="${study.name.replace(/[^a-z0-9_\-\. ]/gi, '')}_empty_export.csv"`,
        },
      });
    }
    ...
    const csvHeaders = [
      ...
      'optionId',
      'optionText',
      'rawAnswerValue', // For more complex answer structures
    ...
    for (const res of responses) {
      ...
      const demographicsForRow = sortedDemographicKeys.map(key => {
        const value =
          res.demographics && typeof res.demographics === 'object'
            ? (res.demographics as Prisma.JsonObject)[key]
            : null;
      ...
      if (Array.isArray(res.answers) && res.answers.length > 0) {
        for (const answer of res.answers as Prisma.JsonArray) {
          if (typeof answer === 'object' && answer !== null && 'questionId' in answer) {
            ...
            const question = res.study.questions.find(
              (q: SurveyQuestion) => q.id === typedAnswer.questionId
            );
