...
async function verifyStudyAccess(studyId: string, orgId: string): Promise<StudyContextForPrompt> {
  const study = await db.brandLiftStudy.findFirst({
    ...
    select: {
      id: true, name: true, funnelStage: true, primaryKpi: true, secondaryKpis: true, status: true,
      campaign: {
        select: {
          campaignName: true,
        }
  ...
  if (currentStatus !== BrandLiftStudyStatus.DRAFT && currentStatus !== BrandLiftStudyStatus.PENDING_APPROVAL) {
    throw new ForbiddenError(`AI suggestions cannot be generated for study status: ${study.status}`);
  }
