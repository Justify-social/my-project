    ...
    try {
        ...
        if (study.cintProjectId && study.cintTargetGroupId) {
            try {
                ...
                cintProgress = await cintService.getTargetGroupOverview(study.cintProjectId, study.cintTargetGroupId);
                logger.info('Fetched live progress from Cint', { studyId, cintProjectId: study.cintProjectId, cintTargetGroupId: study.cintTargetGroupId });
            } catch (cintError: any) {
                logger.warn('Failed to fetch live progress from Cint', { studyId, error: cintError.message });
