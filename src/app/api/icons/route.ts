...
export async function GET() {
  try {
    ...
    if (fs.existsSync(lightIconsPath)) {
      ...
      const invalidLightIcons = lightIcons.filter(
        icon => !validateSvgContent(path.join(lightIconsPath, `${icon}.svg`))
      );

      if (invalidLightIcons.length > 0) {
        warnings.push(
          `Invalid or corrupt light SVG files (${invalidLightIcons.length}): ${invalidLightIcons.join(', ')}`
        );
      }
    } else {
    ...
    if (fs.existsSync(solidIconsPath)) {
      ...
      const invalidSolidIcons = solidIcons.filter(
        icon => !validateSvgContent(path.join(solidIconsPath, `${icon}.svg`))
      );

      if (invalidSolidIcons.length > 0) {
        warnings.push(
          `Invalid or corrupt solid SVG files (${invalidSolidIcons.length}): ${invalidSolidIcons.join(', ')}`
        );
      }
    } else {
    ...
    if (missingRequiredInLight.length > 0) {
      errors.push(
        `Essential icons missing light variants (${missingRequiredInLight.length}): ${missingRequiredInLight.join(', ')}`
      );
    }

    if (missingRequiredInSolid.length > 0) {
      errors.push(
        `Essential icons missing solid variants (${missingRequiredInSolid.length}): ${missingRequiredInSolid.join(', ')}`
      );
    }
    ...
    console.log(
      `Found ${validIconNames.length} valid icon pairs (have both light and solid variants)`
    );
    ...
    if (identicalIcons.length > 0) {
      warnings.push(
        `Icons with identical light/solid variants (${identicalIcons.length}): ${identicalIcons.join(', ')}`
      );
    }
