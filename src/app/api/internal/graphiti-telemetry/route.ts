...
import { logError as _logError } from '@/lib/error-logging';
...
export async function GET(_req: NextRequest) {
  try {
    ...
    const stats = {
      ...
      checkCount: telemetryData.telemetry.filter((t: { action: string }) => t.action === 'check')
        .length,
      blockCount: telemetryData.telemetry.filter((t: { action: string }) => t.action === 'block')
        .length,
      completeCount: telemetryData.telemetry.filter(
        (t: { action: string }) => t.action === 'complete'
      ).length,
      bypassCount: telemetryData.telemetry.filter((t: { action: string }) => t.action === 'bypass')
        .length,
