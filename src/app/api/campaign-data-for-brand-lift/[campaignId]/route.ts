...
import { UnauthenticatedError, ForbiddenError, NotFoundError, BadRequestError, DatabaseError } from '@/lib/errors';
import { handleApiError } from '@/lib/apiErrorHandler';
import { SubmissionStatus } from '@prisma/client';

export async function GET(req: NextRequest, { params: paramsPromise }: { params: Promise<{ campaignId: string }> }) {
    return tryCatch(async () => {
        ...
        logger.info('Fetching campaign by ID for Brand Lift setup', { userId, orgId, campaignId: campaignIdNum });
        ...
        logger.info('Successfully fetched campaign by ID', { userId, orgId, campaignId: campaignIdNum });
