...
export const GET = async (req: NextRequest) => {
    try {
        ...
        if (sessionClaims?.['metadata.role'] !== 'super_admin') {
            logger.warn(
                `User lacking Super Admin role attempted to access Mux assets list.`,
        ...
        logger.info(`Fetched ${assetsToReturn.length} Mux-related creative assets for debug view.`, { userId: clerkUserId });
