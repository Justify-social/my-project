...
export const GET = async (req: NextRequest) => {
    try {
        ...
        if (sessionClaims?.['metadata.role'] !== 'super_admin') {
            logger.warn(
                `User lacking Super Admin role attempted to access campaign wizards list.`,
        ...
        const campaignWizards = await db.campaignWizard.findMany({
            ...
            select: {
                ...
                orgId: true,
                userId: true,
                user: { // User who created the wizard
        ...
        logger.info(`Fetched ${campaignWizards.length} campaign wizards for debug view.`, { userId: clerkUserId });
