...
export const GET = async (req: NextRequest) => {
  try {
    ...
    if (sessionClaims?.['metadata.role'] !== 'super_admin') {
      logger.warn('Non-Super Admin attempted to access /api/admin/organizations', {
        clerkUserId,
    ...
    if (!Array.isArray(organizationList)) {
      logger.error('Unexpected response structure from getOrganizationList', {
    ...
    logger.info('Fetched organization list for Super Admin', {
