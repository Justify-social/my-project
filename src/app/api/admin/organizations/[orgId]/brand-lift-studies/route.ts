    ...
    try {
        ...
        const brandLiftStudies = await db.brandLiftStudy.findMany({
            ...
            select: {
                ...
                campaign: { // This is the CampaignWizardSubmission
                    select: {
                        campaignName: true, // Name from CampaignWizardSubmission
                        wizard: { // This is the CampaignWizard
                            select: {
        ...
        logger.info(`Fetched ${studiesToReturn.length} brand lift studies for organisation ${orgId}`, { orgId, userId: clerkUserId });
