...
function parseUploadThingToken(): { appId: string; apiKey: string } | null {
  try {
    ...
    try {
      ...
      console.log(`Valid credentials found: appId=${decoded.appId}, apiKey=${decoded.apiKey.substring(0, 5)}...`);
      return {
        appId: decoded.appId,
  ...
  try {
    ...
    if (!credentials) {
      ...
      if (process.env.UPLOADTHING_SECRET && process.env.UPLOADTHING_APP_ID) {
        ...
        console.log(`Trying legacy API endpoint: ${url.replace(process.env.UPLOADTHING_SECRET || '', '[REDACTED]')}`);
    ...
    if (response.ok) {
      ...
      try {
        ...
        if (status === 400 || status === 401) {
          ...
          console.log(`Trying admin API endpoint: ${adminUrl.replace(credentials.apiKey, '[REDACTED]')}`);
...
async function findUploadThingFileKeys(requestedFileId: string): Promise<string[]> {
  try {
    ...
    try {
      ...
      console.log('Direct URL check failed, falling back to API:', directError instanceof Error ? directError.message : String(directError));
    }
    ...
    if (keys.length === 0 && requestedFileId && requestedFileId.length > 10) {
      console.log(`No matches in API but returning original ID for possible new upload: ${requestedFileId}`);
      return [requestedFileId];
    }
...
async function handleAssetProxy(request: Request, isHeadRequest = false) {
  ...
  const isUploadThingUrl = assetUrl.includes('ufs.sh') || 
    assetUrl.includes('uploadthing') || 
    assetUrl.includes('utfs.io');
    ...
    if (isUploadThingUrl && extractedFileId) {
      ...
      if (extractedFileId) {
        try {
          ...
          console.log(`❌ Direct URL access error: ${directError instanceof Error ? directError.message : String(directError)}`);
        }
      }
      ...
      if (cachedFormat && (Date.now() - cachedFormat.timestamp < CACHE_TTL)) {
        ...
        try {
          ...
          if (response.ok) {
            ...
            return isHeadRequest ? 
              new Response(null, { status: 200 }) :
              fetch(cachedFormat.url);
          } else {
            console.log(`❌ Cached URL format is no longer valid, trying alternatives`);
          }
        } catch (error) {
          console.log(`❌ Error fetching cached URL: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
      ...
      if (!credentials) {
        ...
        try {
          ...
          console.log(`❌ Direct URL failed: ${error instanceof Error ? error.message : String(error)}`);
        }
      ...
      if (potentialFileKeys.length === 0) {
        ...
        console.log(`🗑️ File confirmed as permanently deleted after exhaustive checks: ${extractedFileId}`);
      ...
      for (const potentialKey of potentialFileKeys) {
        ...
        console.log(`Trying ${urlFormats.length} different URL formats for file key: ${potentialKey}`);
      ...
      try {
        ...
        console.log(`❌ Direct proxy error: ${error instanceof Error ? error.message : String(error)}`);
