...
const MuxAssetCheckerPage: React.FC = () => {
            ...
            const fetchAssets = async () => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        ...
                        throw new Error(errData.error || `API Error fetching Mux assets: ${response.status} ${response.statusText}`);
                    }
    ...
    const getMuxStatusBadgeVariant = (status: string | null | undefined): (
        | 'default'
        | 'destructive'
        | 'secondary'
        | 'outline'
    ...
    if (!isAuthLoaded) {
        return <div className="container mx-auto p-4 md:p-6 text-center"><LoadingSpinner /></div>;
    }
                                    ...
                                    {assets.map(asset => (
                                        <TableRow key={asset.id}>
                                            <TableCell>
                                                ...
                                                {asset.muxAssetId && <div className="text-xs text-muted-foreground">Mux Asset ID: {asset.muxAssetId}</div>}
                                            </TableCell>
                                            <TableCell>
                                                {asset.campaignName ? (
                                                    <Link href={`/campaigns/${asset.campaignId}`} target="_blank" rel="noopener noreferrer" className="text-sm hover:underline">
                                                        {asset.campaignName}
                                                    </Link>
                                            ...
                                            <TableCell className="text-right">
                                                {asset.type === 'video' && asset.muxPlaybackId && (
                                                    <Button asChild variant="outline" size="sm">
                                                        <a href={`https://stream.mux.com/${asset.muxPlaybackId}.m3u8`} target="_blank" rel="noopener noreferrer">
