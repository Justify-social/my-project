    ...
    <ErrorBoundary>
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="flex flex-col md:flex-row gap-6 bg-white p-4 rounded shadow">
          ...
          <div className="flex-1 flex flex-col justify-between">
            <div>
              ...
              {influencer.bio && <p className="mt-2 text-gray-700">{influencer.bio}</p>}

              <div className="mt-4 space-y-2">
                <div>
                  <span className="font-semibold">Platform:</span> {influencer.platform}
                </div>
                <div>
                ...
                <div>
                  <span className="font-semibold">Dynamic Justify Score:</span>{" "}
                  {influencer.justifyScore}{" "}
                  {influencer.tier && (
                    <span className="text-sm text-gray-500">({influencer.tier})</span>
                  )}
                </div>
                {typeof influencer.brandSafetyRating === "number" && (
                  <div>
                    <span className="font-semibold">Brand Safety Rating:</span>{" "}
            ...
            {influencer.socialStats && (
              <div className="mt-6 bg-gray-50 p-3 rounded">
                <h2 className="font-semibold mb-2">Social Stats</h2>
                <div className="flex space-x-6">
                  <div>
                    <span className="font-bold">{influencer.socialStats.followers}</span>{" "}
                    <span className="text-sm text-gray-500">Followers</span>
                  </div>
                  <div>
                    <span className="font-bold">{influencer.socialStats.following}</span>{" "}
                    <span className="text-sm text-gray-500">Following</span>
                  </div>
                  <div>
                    <span className="font-bold">{influencer.socialStats.totalPosts}</span>{" "}
          ...
          <div className="mt-8 bg-white p-4 rounded shadow">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {influencer.recentCampaigns.map((campaign) => (
                <div key={campaign.id} className="border rounded p-2 hover:shadow transition">
                  <img
                    src={campaign.image}
                    alt={campaign.title}
                    className="w-full h-32 object-cover rounded mb-2"
                  />
                  <p className="font-semibold text-sm sm:text-base">{campaign.title}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {influencer.reviews && influencer.reviews.length > 0 && (
          <div className="mt-8 bg-white p-4 rounded shadow">
            <h2 className="text-2xl font-bold mb-4">Peer Reviews</h2>
            <ul className="space-y-4">
              {influencer.reviews.map((review, index) => (
                <li key={index} className="border-b pb-4 last:border-none last:pb-0">
