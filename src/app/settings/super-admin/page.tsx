...
const SuperAdminSettingsPage = () => {
    const { isLoaded: isUserLoaded, user } = useUser(); // Need user for checks
    ...
    // Add back state for user detail modal if needed
    // const [selectedUser, setSelectedUser] = useState<UserData | null>(null);
    // const [userModalOpen, setUserModalOpen] = useState(false);
    // const [isUserDetailLoading, setIsUserDetailLoading] = useState(false);
    // const [userDetailError, setUserDetailError] = useState<string | null>(null);
    const router = useRouter(); // Keep for debug button
    ...
    // --- Render Logic --- 
    if (!isAuthLoaded || !isUserLoaded) { // Wait for both user and auth
    ...
    return (
        <div className="space-y-8">
            ...
            {isLoadingData ? (
                ...
                <div className="p-4 text-destructive bg-destructive/10 border border-destructive rounded-md">Error loading users: {error}</div>
            ) : (
                <Card>
                    <CardHeader>
                                ...
                                {users.map(userData => (
                                    <TableRow key={userData.id} className="hover:bg-muted/50">
                                        ...
                                        <TableCell className="text-right">
                                            <Button variant="ghost" size="sm" onClick={() => handleViewUser(userData)} className="mr-2">View</Button>
                                            {!isUserSuspended(userData.id) ? (
                                                <Button variant="ghost" size="sm" className="text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => { setUserToSuspend(userData); setShowSuspendConfirm(true); }}>
                                                    Suspend
                                                </Button>
                                            ) : (
            ...
            <Dialog open={showSuspendConfirm} onOpenChange={setShowSuspendConfirm}>
                <DialogContent>
                    ...
                    <div className="py-4">
                        <p>Are you sure you want to suspend <strong>{userToSuspend?.name}</strong> ({userToSuspend?.email})?</p>
                        <p className="mt-2 text-sm text-muted-foreground">This action can be reversed later.</p>
                    </div>
                    <DialogFooter>
                        <Button variant="ghost" onClick={() => setShowSuspendConfirm(false)} disabled={isSuspending}>Cancel</Button>
