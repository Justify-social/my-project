...
const OrganisationDetailPage: React.FC = () => {
            ...
            const fetchDataForOrg = async () => {
                ...
                try {
                    ...
                    const [orgDetailsRes, usersRes, wizardsRes, studiesRes] = await Promise.all([
                        fetch('/api/admin/organizations').then(res => res.ok ? res.json() : Promise.reject(new Error('Failed to fetch org list for name'))),
                    ...
                    if (!usersRes.ok) {
                        ...
                        throw new Error(errData.error || `API Error fetching users: ${usersRes.status} ${usersRes.statusText}`);
                    }
                    ...
                    if (!wizardsRes.ok) {
                        ...
                        throw new Error(errData.error || `API Error fetching campaign wizards: ${wizardsRes.status} ${wizardsRes.statusText}`);
                    }
                    ...
                    if (!studiesRes.ok) {
                        ...
                        throw new Error(errData.error || `API Error fetching brand lift studies: ${studiesRes.status} ${studiesRes.statusText}`);
                    }
                    setBrandLiftStudies(await studiesRes.json() || []);

                } catch (err: unknown) {
                    const errorMessage = err instanceof Error ? err.message : String(err);
                    logger.error(`Error fetching data for organisation ${orgId}:`, { error: errorMessage, orgId });
    ...
    if (!isSuperAdmin && isAuthLoaded) {
        return <div className="container mx-auto p-4 md:p-6">Access Denied. Super Admin role required.</div>;
    }
    ...
    return (
        <div className="container mx-auto p-4 md:p-6 space-y-8">
            <Button variant="outline" onClick={() => router.push('/settings/super-admin')} className="mb-4">
                            ...
                            <TableBody>
                                {users.map(user => (
                                    <TableRow key={user.id}>
                                        <TableCell>
                                            {user.profileImageUrl && <img src={user.profileImageUrl} alt="avatar" className="w-8 h-8 rounded-full inline-block mr-2" />}
            ...
            <Card>
                ...
                <CardContent>
                    {campaignWizards.length > 0 ? (
                        ...
                        <p className="text-muted-foreground">No campaign wizards found for this organisation.</p>
                    )}
                </CardContent>
            </Card>
            ...
            <Card>
                ...
                <CardContent>
                    {brandLiftStudies.length > 0 ? (
                        ...
                        <p className="text-muted-foreground">No brand lift studies found for this organisation.</p>
                    )}
                </CardContent>
            </Card>
