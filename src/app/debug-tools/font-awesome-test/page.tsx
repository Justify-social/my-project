...
const SafeFontAwesomeIcon = ({
  ...
  try {
    ...
    if (!icon || Array.isArray(icon) && (!icon[0] || !icon[1])) {
      ...
      return <SafeQuestionMarkIcon className={className} {...props} solid={false} />;
    }
    return <FontAwesomeIcon icon={icon} className={className} {...props} solid={false} />;
  } catch (error) {
...
const testIcons = ['user', 'house', 'magnifying-glass', 'gear', 'chart-pie', 'envelope', 'bell', 'check'] as const;

// List of social media icons to test
const socialIcons = ['twitter', 'facebook', 'instagram', 'youtube', 'linkedin', 'tiktok', 'reddit', 'github'] as const;
const FontAwesomeTest = () => {
  ...
  const [customIconResult, setCustomIconResult] = useState<{
    success: boolean;
    message: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined' && 'performance' in window) {
      const observer = new PerformanceObserver(list => {
        const entries = list.getEntries().filter(entry => entry.name.includes('fontawesome') || entry.name.includes('fa-')).map(entry => ({
          name: entry.name,
  ...
  useEffect(() => {
    ...
    const checkRenderedIcons = setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        if (kitLoaded && (window as any).FontAwesomeKitConfig?.license === 'pro' && (iconClassesWork || iconComponentsWork)) {
  ...
  return <div className="container mx-auto p-8">
      ...
      backgroundColor: fontAwesomeHealth === 'healthy' ? '#d1fae5' : fontAwesomeHealth === 'degraded' ? '#fef3c7' : fontAwesomeHealth === 'failing' ? '#fee2e2' : '#f3f4f6',
      border: '1px solid',
      borderColor: fontAwesomeHealth === 'healthy' ? '#6ee7b7' : fontAwesomeHealth === 'degraded' ? '#fcd34d' : fontAwesomeHealth === 'failing' ? '#fca5a5' : '#e5e7eb'
    }}>
        <div className="flex items-center mb-2">
          <h2 className="text-lg font-medium">System Status:</h2>
          <span className="ml-2 font-medium" style={{
          color: fontAwesomeHealth === 'healthy' ? '#047857' : fontAwesomeHealth === 'degraded' ? '#92400e' : fontAwesomeHealth === 'failing' ? '#b91c1c' : '#374151'
        }}>
            {fontAwesomeHealth === 'healthy' ? 'Working Perfectly ✓' : fontAwesomeHealth === 'degraded' ? 'Working Perfectly ✓' : fontAwesomeHealth === 'failing' ? 'Failing ✗' : 'Checking...'}
          </span>
        </div>
        
        <p style={{
        color: fontAwesomeHealth === 'healthy' ? '#065f46' : fontAwesomeHealth === 'degraded' ? '#92400e' : fontAwesomeHealth === 'failing' ? '#b91c1c' : '#374151'
      }}>
        ...
        {fontAwesomeHealth === 'healthy' && <div className="mt-4 p-3 bg-white rounded border border-green-200">
        ...
        {diagnosticDetails.length > 0 && <div className="mt-2">
            <button onClick={() => document.getElementById('diagnostic-details')?.classList.toggle('hidden')} className="text-xs text-blue-600 hover:text-blue-800 underline">
              View Technical Details
            </button>
            <div id="diagnostic-details" className="hidden mt-2 text-xs bg-gray-50 p-2 rounded-md text-gray-700">
              <ul className="list-disc list-inside">
                {diagnosticDetails.map((detail, index) => <li key={index}>{detail}</li>)}
              </ul>
            </div>
          </div>}
      </div>
...
library.add(faUser, faCheck, faTwitter);`}
        ...
        <section className="bg-white p-6 rounded-lg shadow-md">
          ...
          <div className="grid grid-cols-1 gap-3">
            {Object.entries(testResults).map(([test, result]) => <div key={test} className="flex items-center justify-between p-2 border-b">
      ...
      <section className="mb-8 bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-4">
          <div className="flex-1">
            ...
            <input type="text" className="w-full px-3 py-2 border rounded-md" placeholder="Enter icon name (e.g. user, heart)" value={customIconTest} onChange={e => setCustomIconTest(e.target.value)} />
          </div>
          <div className="md:w-1/4">
            <label className="block text-sm font-medium mb-1">Icon Style</label>
            <select className="w-full px-3 py-2 border rounded-md" value={customIconStyle} onChange={e => setCustomIconStyle(e.target.value)}>
          ...
          <div className="md:w-1/4 flex items-end">
            <button className="w-full px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700" onClick={handleCustomIconTest}>
              Test Icon
            </button>
          </div>
        </div>
        
        {customIconResult && <div className={`p-3 rounded-md mt-3 ${customIconResult.success ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
            <p className={customIconResult.success ? 'text-green-700' : 'text-red-700'}>
              {customIconResult.message}
            </p>
            {customIconResult.success && <div className="mt-2 flex justify-center">
                <div className="p-4 bg-white rounded-md border">
                  <SafeFontAwesomeIcon icon={[customIconStyle as any, customIconTest]} className="w-8 h-8" solid={false} />
                </div>
              </div>}
          </div>}
      </section>
      ...
      <section className="bg-white p-6 rounded-lg shadow-md">
        ...
        {networkRequests.length > 0 ? <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {networkRequests.map((req, i) => <tr key={i} className="border-b">
          ...
          </div> : <p className="text-gray-500">No Font Awesome network requests detected.</p>}
      </section>
