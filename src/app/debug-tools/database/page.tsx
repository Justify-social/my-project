...
export default function DatabasePage() {
  ...
  const clearSlowQueries = async () => {
    try {
      await fetch('/api/health/db/clear-slow-queries', { method: 'POST' });
      // Refresh data
      const response = await fetch('/api/health/db?extended=true');
      const data = await response.json();
      setDbHealth(data);
      ...
      <div className="container mx-auto p-6 max-w-5xl">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <Link 
              href="/debug-tools"
              className="mt-4 inline-block px-4 py-2 bg-[var(--accent-color)] text-white rounded-md hover:opacity-90"
            >
        ...
        <div className="flex flex-wrap items-center justify-between mb-4">
          ...
          <div className="flex mt-2 md:mt-0">
            <button 
              ...
              className={`px-3 py-1.5 mr-2 text-sm rounded ${activeSection === 'overview' 
                ? 'bg-[var(--accent-color)] text-white' 
                : 'bg-gray-100 text-[var(--secondary-color)]'}`}
            >
              Overview
            </button>
            <button 
              onClick={() => setActiveSection('performance')}
              className={`px-3 py-1.5 mr-2 text-sm rounded ${activeSection === 'performance' 
                ? 'bg-[var(--accent-color)] text-white' 
                : 'bg-gray-100 text-[var(--secondary-color)]'}`}
            >
              Performance
            ...
            <button 
              ...
              className={`px-3 py-1.5 mr-2 text-sm rounded ${activeSection === 'transactions' 
                ? 'bg-[var(--accent-color)] text-white' 
                : 'bg-gray-100 text-[var(--secondary-color)]'}`}
            >
              Transactions
            ...
            <button 
              ...
              className={`px-3 py-1.5 text-sm rounded ${activeSection === 'test' 
                ? 'bg-[var(--accent-color)] text-white' 
                : 'bg-gray-100 text-[var(--secondary-color)]'}`}
            >
              Test DB
          ...
          <>
            ...
            {activeSection === 'overview' && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div className={`rounded-lg p-4 border ${
                  ...
                  <div className="flex items-center">
                    <div className={`h-3 w-3 rounded-full mr-2 ${
                      dbHealth.status === 'healthy' ? 'bg-green-500' : 'bg-red-500'
                    }`}></div>
                ...
                <div className="rounded-lg p-4 bg-[var(--background-color)] border border-[var(--divider-color)]">
                  ...
                  <div className="mt-4">
                    <button 
                      onClick={() => fetch('/api/health/db?extended=true').then(r => r.json()).then(data => setDbHealth(data))}
                      className="px-3 py-1 bg-[var(--accent-color)] text-white text-sm rounded hover:opacity-90 transition-opacity"
                    >
                      Refresh Data
                    </button>
                  </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-[var(--background-color)] rounded-lg border border-[var(--divider-color)] p-4">
                  ...
                  {dbHealth.slowQueries && dbHealth.slowQueries.length > 0 ? (
                      ...
                      <table className="min-w-full divide-y divide-[var(--divider-color)]">
                        ...
                        <tbody className="divide-y divide-[var(--divider-color)]">
                          {dbHealth.slowQueries.map((query, index) => (
                            <tr key={index} className={index % 2 === 0 ? 'bg-[var(--background-color)]' : 'bg-[var(--background-light-color)]'}>
                              <td className="px-3 py-2 text-sm">{query.operation}</td>
                              <td className="px-3 py-2 text-sm">{query.model}</td>
                              <td className={`px-3 py-2 text-sm ${
                                query.duration > 1000 ? 'text-red-600' : 
                                query.duration > 500 ? 'text-amber-600' : 
                                'text-yellow-600'
                              }`}>
                    ...
                    <p className="text-[var(--secondary-color)] text-sm">No slow queries recorded.</p>
                  )}
                </div>
                  ...
                  <>
                      ...
                      <div className="bg-[var(--background-color)] rounded-lg border border-[var(--divider-color)] p-4 md:col-span-2">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                          {Object.entries(dbHealth.transactions.byOperation).map(([op, stats]) => (
                            <div key={op} className="bg-[var(--background-light-color)] p-2 rounded">
                    ...
                    <div className="bg-[var(--background-color)] rounded-lg border border-[var(--divider-color)] p-4">
                      ...
                      {dbHealth.transactions.recentTransactions.length > 0 ? (
                          ...
                          <table className="min-w-full divide-y divide-[var(--divider-color)]">
                            ...
                            <tbody className="divide-y divide-[var(--divider-color)]">
                              {dbHealth.transactions.recentTransactions.map((tx, index) => (
                                <tr key={tx.id} className={index % 2 === 0 ? 'bg-[var(--background-color)]' : 'bg-[var(--background-light-color)]'}>
                        ...
                        <p className="text-[var(--secondary-color)] text-sm">No recent transactions recorded.</p>
                      )}
                    </div>
                  </>
                ) : (
            ...
            {activeSection === 'test' && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div 
                    className="bg-[var(--background-color)] rounded-lg border border-[var(--divider-color)] p-4 hover:shadow-md transition-shadow"
                  >
                    ...
                    <button
                      ...
                      className="w-full px-4 py-2 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 transition-colors disabled:opacity-50"
                    >
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={isRunningTest}
                      className="w-full px-4 py-2 bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50"
                    >
                      {isRunningTest ? 'Running...' : 'Run Test'}
                    </button>
                  </div>
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={isRunningTest}
                      className="w-full px-4 py-2 bg-green-100 text-green-700 rounded hover:bg-green-200 transition-colors disabled:opacity-50"
                    >
                      {isRunningTest ? 'Running...' : 'Run Test'}
                    </button>
                  </div>
      ...
      <div className="mb-6">
        <div className="flex flex-wrap gap-2 border-b border-[var(--divider-color)]">
          <button 
            ...
            className={`px-4 py-2 ${activeTab === 'all' ? 'border-b-2 border-[var(--accent-color)] text-[var(--accent-color)]' : 'text-[var(--secondary-color)]'}`}
          >
            All Documents
          </button>
          <button 
            onClick={() => setActiveTab('database')}
            className={`px-4 py-2 ${activeTab === 'database' ? 'border-b-2 border-[var(--accent-color)] text-[var(--accent-color)]' : 'text-[var(--secondary-color)]'}`}
          >
            Database
          </button>
          <button 
            onClick={() => setActiveTab('linter')}
            className={`px-4 py-2 ${activeTab === 'linter' ? 'border-b-2 border-[var(--accent-color)] text-[var(--accent-color)]' : 'text-[var(--secondary-color)]'}`}
          >
            Linter Reports
          </button>
          <button 
            onClick={() => setActiveTab('api')}
            className={`px-4 py-2 ${activeTab === 'api' ? 'border-b-2 border-[var(--accent-color)] text-[var(--accent-color)]' : 'text-[var(--secondary-color)]'}`}
          >
            API Documentation
          </button>
          <button 
            onClick={() => setActiveTab('general')}
            className={`px-4 py-2 ${activeTab === 'general' ? 'border-b-2 border-[var(--accent-color)] text-[var(--accent-color)]' : 'text-[var(--secondary-color)]'}`}
          >
            General
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredDocs.map((doc) => (
          <a 
            ...
            className="bg-[var(--background-color)] rounded-lg border border-[var(--divider-color)] p-6 shadow-sm hover:shadow transition-shadow"
          >
            ...
            <div className="mt-4 text-right">
              <span className="text-xs px-2 py-1 bg-gray-100 rounded-full text-[var(--secondary-color)]">
                {doc.category === 'database' ? 'Database' : 
                 doc.category === 'linter' ? 'Linter Report' : 
                 doc.category === 'api' ? 'API Docs' : 'General'}
              </span>
            </div>
          </a>
        ))}
...
function RunScriptButton({ scriptName, label, description }: { 
  scriptName: string;
  label: string;
  description: string;
}) {
  ...
  return (
    <div className="flex flex-col h-full bg-amber-50 border border-amber-200 rounded-md p-4">
      ...
      {isRunning ? (
        ...
        <div className={`py-2 px-3 rounded-md text-sm ${
        ...
        <button
          onClick={runScript}
          className="mt-auto px-4 py-2 bg-amber-200 text-amber-800 rounded hover:bg-amber-300 transition-colors focus:outline-none focus:ring-2 focus:ring-amber-400 focus:ring-offset-2"
        >
          Run Script
        </button>
      )}
