...
/**
 * Higher-order function to wrap a handler with database error handling
 * 
 * @param handler The handler function to wrap
 * @param entityName The name of the entity being operated on
 * @param operation The database operation being performed
 * @returns A function that handles database errors
 */
export function withDbErrorHandling<T>(
  ...
  return handler()
    .then(result => result)
    .catch(error => handleDbError(error, entityName, operation));
}
