...
export default function SurveyApprovalContent() {
  ...
  return <div className="min-h-screen bg-white p-4 md:p-8 font-['Work_Sans']">
      <div className="max-w-7xl mx-auto">
        <div className="mb-4 flex justify-between items-center">
          ...
          <button className="px-4 py-2 bg-[var(--accent-color)] text-white rounded-md hover:bg-[rgba(0,191,255,0.8)] transition-colors" onClick={() => console.log('Request formal sign-off')}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            ...
            {/* Survey Questions */}
            {surveys[0].questions.map(question => <div key={question.id} className="bg-white rounded-lg shadow-md mb-4 overflow-hidden border border-[var(--divider-color)]">
                <div className="p-4 cursor-pointer flex justify-between items-center border-b border-[var(--divider-color)]" onClick={() => toggleQuestion(question.id)}>
                  ...
                  {question.expanded ? <Icon name="faChevronDown" className="h-5 w-5 text-[var(--secondary-color)]" solid={false} /> : <Icon name="faChevronRight" className="h-5 w-5 text-[var(--secondary-color)]" solid={false} />}
                </div>
                
                {question.expanded && <div className="p-4">
                    {question.id === 'q1' ? <div>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                          {question.options.map(option => <div key={option.id} className="relative">
                              <div className={`border border-[var(--divider-color)] rounded-lg overflow-hidden cursor-pointer transition-all ${selectedOptions[question.id] === option.id ? 'ring-2 ring-[var(--accent-color)]' : 'hover:shadow-md'}`} onClick={() => handleOptionSelect(question.id, option.id)}>

                                <div className="h-32 relative">
                                  {option.image && <div className="h-full w-full bg-gray-200 relative">
                                ...
                                <div className="p-2 border-t border-[var(--divider-color)]">
                                  <div className="flex items-center">
                                    <div className={`w-4 h-4 rounded-full border flex-shrink-0 mr-2 flex items-center justify-center ${selectedOptions[question.id] === option.id ? 'border-[var(--accent-color)]' : 'border-gray-300'}`}>
                                      {selectedOptions[question.id] === option.id && <div className="w-2 h-2 rounded-full bg-[var(--accent-color)]"></div>}
                                    </div>
                        ...
                        {question.options.map(option => <div key={option.id} className={`p-3 border border-[var(--divider-color)] rounded-md flex items-center cursor-pointer ${selectedOptions[question.id] === option.id ? 'border-[var(--accent-color)] bg-[rgba(0,191,255,0.1)]' : 'hover:bg-gray-50'}`} onClick={() => handleOptionSelect(question.id, option.id)}>
            ...
            <button className="w-full py-3 bg-[var(--accent-color)] text-white rounded-md hover:bg-[rgba(0,191,255,0.8)] transition-colors font-medium" onClick={() => {
          ...
          <div className="lg:col-span-1">
            <div className="bg-white p-6 rounded-lg shadow-md border border-[var(--divider-color)]">
              ...
              <div className="space-y-6">
                {reviewers.map(reviewer => <div key={reviewer.id} className="border-b border-[var(--divider-color)] pb-4 last:border-b-0 last:pb-0">
                    ...
                    <div className="flex justify-end">
                      {reviewer.status === 'Need Action' ? <div className="flex space-x-2">
                          <button className="p-1" onClick={() => console.log('Edit comment')}>
                            <Icon name="faEdit" className="h-4 w-4 text-gray-400" solid={false} />
                          </button>
                          <button className="px-3 py-1 bg-gray-800 text-white text-xs rounded-md hover:bg-gray-700" onClick={() => implementSuggestion(reviewer.id)}>

                            Implement Suggestion
                          </button>
                        </div> : <div className="flex items-center text-sm">
                          <Icon name="faCircleCheck" className="h-5 w-5 text-green-500" solid={false} />
