  ...
  return (
    <div className="sankey-container">
      <div className="mb-3 flex justify-between items-center">
        <h3 className="text-xl font-semibold text-gray-800">Customer Journey Flow</h3>
        <div className="flex gap-2">
          <button className="text-sm px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors">
            Export
          </button>
          <select className="text-sm px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded-md">
      ...
      {highlightedNode && (
        <div className="mt-4 p-4 bg-gray-50 border border-gray-200 rounded-lg">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <h5 className="text-sm font-medium text-gray-600">Incoming Connections</h5>
                  ...
                  .map((link, i) => {
                    ...
                    return (
                      <li key={i} className="text-sm">
                        <span className="font-medium">{sourceNode?.name}</span>: {link.value.toLocaleString()} users
            ...
            <div>
              <h5 className="text-sm font-medium text-gray-600">Outgoing Connections</h5>
              <ul className="mt-1 space-y-1">
                {data.links
                  ...
                  .map((link, i) => {
                    ...
                    return (
                      <li key={i} className="text-sm">
                        <span className="font-medium">{targetNode?.name}</span>: {link.value.toLocaleString()} users
                        {link.conversionRate && ` (${(link.conversionRate * 100).toFixed(1)}% conversion)`}
                      </li>
                    );
                  })}
      ...
      <div className="mt-3 text-sm text-gray-500">
        <p>The Sankey diagram visualizes user flow between marketing channels. Click on any node to see detailed information.</p>
      </div>
    </div>
  );
};
