...
import { generateCorrelationId, sanitizeFileName, logAndShowError } from "@/utils/fileUtils";
...
export function CampaignAssetUploader({
  ...
  return <div className="w-full">
      <UploadDropzone<FileRouter, "campaignAssetUploader"> endpoint="campaignAssetUploader" onBeforeUploadBegin={(files: File[]) => {
    ...
    }} className="w-full py-10 flex flex-col items-center justify-center transition-all duration-300 bg-white rounded-xl border border-gray-200 shadow-sm" content={{
      allowedContent: "Images and videos only",
      label: "Choose a file or drag and drop",
      uploadIcon: () => <div className="mb-8 p-6 rounded-full bg-blue-50 flex items-center justify-center mx-auto">
              <Icon name="faUpload" className="h-16 w-16 text-blue-500" solid={false} />
            </div>,
      button: ({
        ready
      }: {
      ...
      selectedFiles.length > 0 && <button className="mt-4 px-6 py-2 text-white font-medium rounded-lg shadow-sm transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-400" style={{
        backgroundColor: 'var(--accent-color)',
        // Using the accent color from globals.css
        fontFamily: "'Work Sans', sans-serif" // Using the app's font
      }} disabled={!ready} onMouseOver={e => {
      ...
      {!isUploading && selectedFiles.length > 0 && <div className="mt-4 space-y-3">
          <p className="text-sm text-gray-500 font-medium">{selectedFiles.length} file(s) selected</p>
          {selectedFiles.map((file, index) => <div key={index} className="bg-white rounded-lg border border-gray-200 p-4">
              <div className="flex items-center">
                <Icon name="faInfo" className="h-6 w-6 text-gray-400 mr-3" solid={false} />
      ...
      {isUploading && <div className="mt-6 flex flex-col items-center">
