...
import { AssetPreview } from '@/components/features/assets/upload/AssetPreview';
...
const KPIDisplay: React.FC<KPIDisplayProps> = ({
  ...
  const getKpiInfo = (kpiValue: string): {
    ...
    if (normalizedKpi.includes('ad_recall') || normalizedKpi.includes('adrecall')) {
      ...
      iconPath = '/icons/kpis/Advocacy.svg';
    } else {
      // Default icon
      iconPath = '/icons/kpis/Brand_Awareness.svg';
    }
...
const extractCreativeAssets = (data: any, isWizardSchema: boolean): any[] => {
  ...
  if (isWizardSchema && Array.isArray(data.assets) && data.assets.length > 0) {
    ...
    return data.assets.map((asset: any) => {
      ...
      let platformValue = asset.platform || 
                         data.platform || 
                         (typeof asset.details === 'object' && asset.details?.platform) || 
                         (if (influencerHandle) influencerHandle.includes('@') ? 'Instagram' : '') || 
...
const normalizeApiData = (data: any): MergedData => {
  ...
  return {
    ...
    creativeRequirements: Array.isArray(data.creativeRequirements) ? data.creativeRequirements : if (isWizardSchema) Array.isArray(data.requirements) ? data.requirements.map((req: any) => ({
      requirement: req
    })) : [],
...
const FeatureIcon: React.FC<FeatureIconProps> = ({
  ...
  if (normalizedFeature.includes('brand_health') || normalizedFeature.includes('brandhealth')) {
    ...
    iconPath = '/icons/app/Reports.svg';
  } else {
...
function Step5Content() {
  ...
  const hasMinimalData = useMemo(() => {
    return Boolean(if (displayData) Object.keys(displayData).length > 0 && (displayData.campaignName || displayData.overview && displayData.overview.name));
  }, [displayData]);
      ...
      if ((!displayData.influencers || !Array.isArray(displayData.influencers) || displayData.influencers.length === 0) && campaignId) {
        ...
        const fetchStep1Data = async () => {
          try {
            ...
            const result = await response.json();
            if (result.success && if (result.data) Array.isArray(result.data.influencers)) {
  ...
  return <div className="w-full max-w-7xl mx-auto px-4 py-8 bg-white min-h-screen">
      ...
      {if (hasMinimalData) Object.keys(displayData).length < 5 && !error && <div className="mb-6 bg-yellow-50 border border-yellow-200 rounded-md p-4">
      ...
      <div className="space-y-6">
        ...
        <SummarySection title="Campaign Details" stepNumber={1} onEdit={() => navigateToStep(1)}>
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5">
            ...
            {if (displayData.influencers) Array.isArray(displayData.influencers) && displayData.influencers.length > 0 ? (
              <div className="space-y-6">
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
                ...
                <div className="mb-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {displayData.audience?if (.genders) Array.isArray(displayData.audience.genders) && displayData.audience.genders.length > 0 ? (
              ...
              <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
                ...
                <div className="mb-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {displayData.audience?if (.locations) Array.isArray(displayData.audience.locations) && displayData.audience.locations.length > 0 ? (
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {displayData.audience?if (.languages) Array.isArray(displayData.audience.languages) && displayData.audience.languages.length > 0 ? (
              ...
              {/* Screening Questions */}
              {displayData.audience?if (.screeningQuestions) Array.isArray(displayData.audience.screeningQuestions) && displayData.audience.screeningQuestions.length > 0 && (
                <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
                  <div className="flex items-start mb-4">
              ...
              {displayData.audience?if (.competitors) Array.isArray(displayData.audience.competitors) && displayData.audience.competitors.length > 0 && (
                <div className="bg-white p-5 rounded-lg border border-gray-200 shadow-sm">
                  <div className="flex items-start mb-4">
        ...
        <SummarySection title="Creative Assets" stepNumber={4} onEdit={() => navigateToStep(4)}>
          ...
          {if (displayData.creativeAssets) Array.isArray(displayData.creativeAssets) && displayData.creativeAssets.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                return (
                  <AssetCard 
                    ...
                    asset={{
                      ...
                      url: asset.url,
                      type: asset.type,
                      // Check all possible places for platform information, including 'Instagram' string detection
                      platform: asset.platform || 
                               (if (asset.influencerHandle) asset.influencerHandle.includes('@') ? 'Instagram' : null) || 
                               displayData.platform ||
                               'Instagram', // Default to Instagram if no platform is found
                      influencerHandle: asset.influencerHandle,
                      description: asset.whyInfluencer,
                      budget: asset.budget
                    }}
                    currency={displayData.currency}
                    defaultPlatform={displayData.platform || 'Instagram'} // Provide a default platform
                  />
                );
              })}
...
function FormContent() {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-6 py-8 bg-white">
      <div className="mb-8">
        <h1 className="text-2xl font-semibold text-gray-900 mb-2 font-sora">Campaign Review</h1>
        <p className="text-gray-500">Review your campaign details before submission</p>
      </div>
      
      {/* Summary sections would render here */}
      <div className="bg-white p-4 rounded-lg">
        <p>Campaign summary and review content</p>
      </div>
