...
export default function AgeDistributionSlider({ values, onChange }: AgeDistributionSliderProps) {
  ...
  const handleSliderChange = (index: number, newValue: number) => {
    ...
    if (diff > 0) {
      const availableToDecrease = newValues.reduce(
    ...
    // If decreasing one value, increase others proportionally
      ...
      const availableToIncrease =
        100 - newValues.reduce((sum, val, i) => (i !== index ? sum + val : sum), 0);

      if (availableToIncrease > 0) {
        const totalOthers = newValues.reduce((sum, val, i) => (i !== index ? sum + val : sum), 0);
    ...
    <div className="mb-6 font-body">
      ...
      <div className="space-y-4 font-body">
        {ageGroups.map((group, index) => (
          <div key={group} className="flex items-center gap-4 font-body">
            <span className="w-16 text-sm font-medium font-body">{group}</span>
            <div className="flex-grow font-body">
              <Slider
                ...
                value={values[index]}
                onChange={value =>
                  handleSliderChange(index, typeof value === 'number' ? value : value[0])
                }
