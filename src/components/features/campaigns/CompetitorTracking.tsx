...
export default function CompetitorTracking({ selected, onChange }: CompetitorTrackingProps) {
  ...
  const allCompanies = ['Company Name 1', 'Company Name 2', 'Company Name 3', 'Company Name 4'];

  const filtered = allCompanies.filter(
    comp => comp.toLowerCase().includes(query.toLowerCase()) && !selected.includes(comp)
  );

  return (
    <div className="mb-4 relative font-body">
      <label className="block font-semibold mb-1 font-body">Search Companies</label>
      <input
        type="text"
