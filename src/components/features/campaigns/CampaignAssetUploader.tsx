...
export function CampaignAssetUploader({
  ...
  const validateFiles = (files: File[]): File[] => {
    ...
    return files.filter(file => {
      ...
      if (!file.type) {
        console.warn(
          `[${correlationId}] File missing type, using fallback detection: ${file.name}`
        );
    ...
    return res
      .map((file, index) => {
        try {
          ...
          const fileName =
            typeof rawName === 'string' ? sanitizeFileName(rawName) : `file-${index}-${Date.now()}`;
  ...
  return (
    <Card>
      <CardContent>
        <UploadDropzone<FileRouter, 'campaignAssetUploader'>
            ...
            return validFiles.map(file => {
              try {
                ...
                if (sanitizedName !== file.name) {
                  console.log(
                    `[${correlationId}] Sanitized file name: ${file.name} -> ${sanitizedName}`
                  );
          ...
          onUploadError={(error: Error) => {
            ...
            if (error.message && error.message.includes('Cannot read properties of undefined')) {
              const betterMessage =
                'Invalid file format or data. Please check your files and try again.';
          ...
          content={{
            ...
            uploadIcon: () => (
              <div className="mb-8 p-6 rounded-full bg-accent/10 flex items-center justify-center mx-auto font-body">
                <Icon iconId="faUploadLight" className="h-16 w-16 text-accent font-body" />
              </div>
            ),
            ...
            button: ({ ready }: { ready: boolean }) =>
              ...
              selectedFiles.length > 0 && (
                <button
                  className="mt-4 px-6 py-2 text-white font-medium rounded-lg shadow-sm transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-400 font-body"
                  style={{
        ...
        {!isUploading && selectedFiles.length > 0 && (
          <div className="mt-4 space-y-3 font-body">
            ...
            {selectedFiles.map((file, index) => (
              <div key={index} className="bg-white rounded-lg border p-4 font-body">
                <div className="flex items-center font-body">
                  <Icon
                    iconId="faInfoLight"
                    className="h-6 w-6 text-muted-foreground mr-3 font-body"
                  />
        ...
        {/* Loading indicator */}
        {isUploading && (
          <div className="mt-4 flex items-center justify-center font-body">
