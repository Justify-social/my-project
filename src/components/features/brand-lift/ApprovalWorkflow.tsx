...
import { Prisma, BrandLiftStudyStatus, SurveyOverallApprovalStatus, SurveyApprovalCommentStatus } from '@prisma/client';
...
import { BrandLiftStudyData, SurveyQuestionData, SurveyApprovalCommentData, SurveyApprovalStatusData } from '@/types/brand-lift';
...
const ApprovalWorkflow: React.FC<ApprovalWorkflowProps> = ({ studyId }) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            if (!studyRes.ok) throw new Error(await studyRes.json().then(e => e.error).catch(() => 'Failed to fetch study details'));
            setStudyData(await studyRes.json());

            if (!questionsRes.ok) throw new Error(await questionsRes.json().then(e => e.error).catch(() => 'Failed to fetch questions'));
            setQuestions((await questionsRes.json() as SurveyQuestionData[]).sort((a, b) => a.order - b.order));

            if (!commentsRes.ok) throw new Error(await commentsRes.json().then(e => e.error).catch(() => 'Failed to fetch comments'));
            ...
            if (approvalStatusRes.ok) {
                ...
                throw new Error(await approvalStatusRes.json().then(e => e.error).catch(() => 'Failed to fetch approval status'));
            }
    ...
    const handleAddComment = async (text: string, questionId?: string | null) => {
        ...
        try {
            ...
            if (!response.ok) throw new Error(await response.json().then(e => e.error).catch(() => 'Failed to add comment'));
    ...
    const handleUpdateOverallStatus = async (newStatus: SurveyOverallApprovalStatus, requestedSignOffUpdate?: boolean) => {
        if (!studyId) return;
        const actionKey = `updateStatus-${newStatus}${requestedSignOffUpdate ? '-reqSignOff' : ''}`;
        setActionLoading(prev => ({ ...prev, [actionKey]: true }));
        setError(null);
        try {
            const payload: { status: SurveyOverallApprovalStatus; requestedSignOff?: boolean } = { status: newStatus };
            if (typeof requestedSignOffUpdate === 'boolean') payload.requestedSignOff = requestedSignOffUpdate;

            const response = await fetch(`/api/brand-lift/approval/status?studyId=${studyId}`, {
                method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload),
            });
            if (!response.ok) throw new Error(await response.json().then(e => e.error).catch(() => 'Failed to update status'));
    ...
    const isStudyApproved = currentOverallStatus === SurveyOverallApprovalStatus.APPROVED || currentOverallStatus === SurveyOverallApprovalStatus.SIGNED_OFF;
    ...
    if (error && !studyData) { // Critical error if studyData itself couldn't load
        return <Alert variant="destructive"><Icon iconId="faTriangleExclamationLight" /> <AlertTitle>Error Loading Approval Data</AlertTitle><AlertDescription>{error}</AlertDescription></Alert>;
    }
            ...
            <div className="lg:col-span-2 space-y-6">
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>Current Main Study Status: <StatusTag status={(studyData?.status as BrandLiftStudyStatus) || 'DEFAULT'} type="study" /></CardDescription>
                        <CardDescription>Approval Status: <StatusTag status={currentOverallStatus} type="approval" /></CardDescription>
                    </CardHeader>
                    <CardContent>
                        ...
                        <div className="space-y-3 max-h-[600px] overflow-y-auto p-1 border rounded bg-slate-50">
                            {questions.length === 0 && <p className="text-muted-foreground p-4 text-center">No questions in this survey.</p>}
                                ...
                                <Card key={q.id} className="p-3 bg-white">
                                    ...
                                    <ul className="list-disc pl-5 mt-1 text-xs text-gray-600 space-y-1">
                                        {q.options.sort((a, b) => a.order - b.order).map(opt => (
                                            <li key={opt.id}>{opt.text} {opt.imageUrl ? "(image)" : ""}</li>
                                        ))}
                                    </ul>
                                </Card>
                            ))}
            ...
            <div className="lg:col-span-1 space-y-6 sticky top-6">
                ...
                <Card>
                    ...
                    <CardContent className="space-y-3">
                        {error && !actionLoading['addComment'] && <Alert variant="destructive"><Icon iconId="faTriangleExclamationLight" className="h-4 w-4" /> <AlertTitle>Action Error</AlertTitle><AlertDescription>{error}</AlertDescription></Alert>}
                        {canApproveOrRequestChanges && (
                            <div className="flex flex-col sm:flex-row gap-2">
                                <Button onClick={() => handleUpdateOverallStatus(SurveyOverallApprovalStatus.APPROVED)} disabled={actionLoading['updateStatus-APPROVED']} className="flex-1 bg-green-600 hover:bg-green-700">
                                    {actionLoading['updateStatus-APPROVED'] ? <Icon iconId="faSpinnerLight" className="animate-spin" /> : <Icon iconId="faCheckLight" className="mr-2" />} Approve Study
                                </Button>
                                <Button variant="outline" onClick={() => handleUpdateOverallStatus(SurveyOverallApprovalStatus.CHANGES_REQUESTED)} disabled={actionLoading['updateStatus-CHANGES_REQUESTED']} className="flex-1">
                                    {actionLoading['updateStatus-CHANGES_REQUESTED'] ? <Icon iconId="faSpinnerLight" className="animate-spin" /> : <Icon iconId="faEditLight" className="mr-2" />} Request Changes
                                </Button>
                            </div>
                        )}
                        {canRequestSignOff && (
                            <Button onClick={() => handleUpdateOverallStatus(SurveyOverallApprovalStatus.APPROVED, true)} disabled={actionLoading['updateStatus-APPROVED-reqSignOff']} className="w-full">
                                {actionLoading['updateStatus-APPROVED-reqSignOff'] ? <Icon iconId="faSpinnerLight" className="animate-spin" /> : <Icon iconId="faFileSignatureLight" className="mr-2" />} Request Final Sign-Off
                            </Button>
                        )}
                        {isSignOffRequested && !isStudySignedOff && !canSignOff && (
                            <p className="text-sm text-muted-foreground text-center p-2 border rounded-md bg-blue-50 border-blue-200">Final sign-off has been requested.</p>
                        )}
                        {canSignOff && (
                            <Button onClick={() => handleUpdateOverallStatus(SurveyOverallApprovalStatus.SIGNED_OFF)} disabled={actionLoading['updateStatus-SIGNED_OFF']} className="w-full bg-emerald-600 hover:bg-emerald-700">
                                {actionLoading['updateStatus-SIGNED_OFF'] ? <Icon iconId="faSpinnerLight" className="animate-spin" /> : <Icon iconId="faStampLight" className="mr-2" />} Final Sign-Off & Lock Survey
                            </Button>
                        )}
                        {isStudySignedOff && (
                    ...
                    {canSubmitForCollection && (
                        <CardFooter className="border-t pt-4">
                            <Button onClick={handleSubmitForDataCollection} className="w-full" disabled={actionLoading['submitCollection']}>
                                {actionLoading['submitCollection'] ? <Icon iconId="faSpinnerLight" className="animate-spin" /> : <Icon iconId="faRocketLight" className="mr-2" />} Submit for Data Collection
                            </Button>
                        </CardFooter>
                    )}
                </Card>
