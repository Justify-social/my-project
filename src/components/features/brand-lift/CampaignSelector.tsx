...
import { Icon } from '@/components/ui/icon'; // Use Icon for potential error icon
        ...
        <Card className="w-full max-w-lg mx-auto">
            ...
            <CardContent className="space-y-4 pt-4">
                ...
                {campaigns.length === 0 && !isLoading && !error && (
                    <p className="text-sm text-muted-foreground text-center py-4">No completed campaigns available.</p>
                )}
                {campaigns.length > 0 && (
                    <Select onValueChange={handleSelectChange} value={selectedCampaignId}>
                        ...
                        <SelectContent>
                            {campaigns.map((campaign) => (
                                <SelectItem key={campaign.id} value={String(campaign.id)}> {/* Value must be string for SelectItem */}
