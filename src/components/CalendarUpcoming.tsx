...
const CalendarUpcoming: React.FC<CalendarUpcomingProps> = ({
  ...
  const renderCalendarView = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(<motion.div key={day} className={`h-24 p-2 border border-gray-100 rounded-lg ${dayEvents.length > 0 ? 'bg-blue-50' : 'bg-white'}`} whileHover={{
        scale: 1.02
      }}>
  ...
  const renderTimelineView = () => {
    ...
    return <div className="space-y-4">
        ...
        {Object.entries(groupedEvents).map(([monthYear, monthEvents]) => <motion.div key={monthYear} initial={{
        opacity: 0,
        y: 20
      }} animate={{
        opacity: 1,
