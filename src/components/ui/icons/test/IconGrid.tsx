...
export function IconGrid() {
  return <div className="p-4 font-work-sans">
      {/* UI Icons Section */}
      <h2 className="text-xl font-bold mb-6 font-sora">UI Icons</h2>
      <p className="mb-4 text-gray-600 font-work-sans">Hover over any icon to see it change from light to solid style with accent color. The group class on the container enables this effect.</p>
      
      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-8 gap-4 mb-8 font-work-sans">
        ...
        return <div key={name} className="group flex flex-col items-center p-3 border rounded-md hover:bg-gray-50 transition-colors cursor-pointer font-work-sans">
              <Icon name={lightIconName} size="md" iconType="button"
          // No custom Tailwind hover class - let the component handle it
          solid={false} className="text-[var(--secondary-color)] font-work-sans" />
              <span className="text-xs mt-2 text-center text-gray-600 font-work-sans">{name}</span>
            </div>;
      })}
      </div>
      
      {/* KPI Icons Section */}
      <h2 className="text-xl font-bold mb-6 mt-12 font-sora">KPI Icons</h2>
      <p className="mb-4 text-gray-600 font-work-sans">KPI-specific icons used throughout the application.</p>
      
      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-8 gap-4 mb-8 font-work-sans">
        {KPI_ICON_NAMES.map((name) => <div key={name} className="group flex flex-col items-center p-3 border rounded-md hover:bg-gray-50 transition-colors cursor-pointer font-work-sans" onMouseOver={(e) => {
        ...
        />
            <span className="text-xs mt-1 text-center text-gray-600 font-work-sans">{formatIconName(name)}</span>
          </div>)}
      </div>
      
      {/* APP Icons Section */}
      <h2 className="text-xl font-bold mb-6 mt-12 font-sora">App Icons</h2>
      <p className="mb-4 text-gray-600 font-work-sans">Application-specific icons used for navigation and features.</p>
      
      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-8 gap-4 mb-8 font-work-sans">
        {APP_ICON_NAMES.map((name) => <div key={name} className="group flex flex-col items-center p-3 border rounded-md hover:bg-gray-50 transition-colors cursor-pointer font-work-sans" onMouseOver={(e) => {
        // Find the img element within this div and apply the filter
        const img = e.currentTarget.querySelector('img');
        if (img) {
          img.style.filter = 'invert(65%) sepia(67%) saturate(2625%) hue-rotate(177deg) brightness(104%) contrast(103%)';
      ...
      <h2 className="text-xl font-bold mb-6 mt-8 font-sora">Icon Action Colors</h2>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-8 font-work-sans">
        <div className="p-4 border rounded-md font-work-sans">
          <h3 className="font-medium mb-3 font-sora">Default Action (Blue)</h3>
          <div className="flex justify-center space-x-4 font-work-sans">
            {["user", "bell", "heart", "star"].map((name) => <div key={name} className="group flex flex-col items-center font-work-sans">
                <Icon name={`fa${name.charAt(0).toUpperCase() + name.slice(1)}Light`} size="lg" iconType="button" action="default" solid={false} />
                <span className="text-xs mt-2 text-gray-600 font-work-sans">{name}</span>
              </div>)}
          </div>
        </div>
        
        <div className="p-4 border rounded-md font-work-sans">
          <h3 className="font-medium mb-3 font-sora">Delete Action (Red)</h3>
          <div className="flex justify-center space-x-4 font-work-sans">
            ...
            return <div key={name} className="group flex flex-col items-center font-work-sans">
                  <Icon name={iconName} size="lg" iconType="button" action="delete" solid={false} />
                  <span className="text-xs mt-2 text-gray-600 font-work-sans">{name.split('-')[0]}</span>
                </div>;
          })}
          </div>
        </div>
        ...
        <div className="p-4 border rounded-md font-work-sans">
          <h3 className="font-medium mb-3 font-sora">Warning Action (Yellow)</h3>
          <div className="flex justify-center space-x-4 font-work-sans">
        ...
        <div className="p-4 border rounded-md font-work-sans">
          <h3 className="font-medium mb-3 font-sora">Success Action (Green)</h3>
          <div className="flex justify-center space-x-4 font-work-sans">
      ...
      <h2 className="text-xl font-bold mb-6 mt-8 font-sora">Static Icons (No Hover Effects)</h2>
      <div className="flex flex-wrap gap-8 mb-8 font-work-sans">
        <div className="p-4 border rounded-md flex-grow font-work-sans">
          <h3 className="font-medium mb-3 font-sora">Light Static Icons</h3>
          <div className="flex justify-center space-x-4 font-work-sans">
            {["user", "bell", "heart", "gear", "check"].map((name) => <div key={name} className="flex flex-col items-center font-work-sans">
                <Icon name={`fa${name.charAt(0).toUpperCase() + name.slice(1)}Light`} size="lg" iconType="static" solid={false} className="text-[var(--secondary-color)] font-work-sans" />
                <span className="text-xs mt-2 text-gray-600 font-work-sans">{name}</span>
              </div>)}
          </div>
        </div>
        ...
        <div className="p-4 border rounded-md flex-grow font-work-sans">
          <h3 className="font-medium mb-3 font-sora">Solid Static Icons</h3>
          <div className="flex justify-center space-x-4 font-work-sans">
            {["user", "bell", "heart", "gear", "check"].map((name) => <div key={name} className="flex flex-col items-center font-work-sans">
                <Icon name={`fa${name.charAt(0).toUpperCase() + name.slice(1)}`} size="lg" iconType="static" solid className="text-[var(--secondary-color)] font-work-sans" />
                <span className="text-xs mt-2 text-gray-600 font-work-sans">{name}</span>
              </div>)}
          </div>
        </div>
      ...
      <div className="mt-12 p-4 bg-gray-50 rounded-md font-work-sans">
        <h3 className="text-lg font-semibold mb-3 font-sora">Usage Tips</h3>
