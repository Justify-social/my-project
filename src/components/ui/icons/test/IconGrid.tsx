...
const SEMANTIC_ICON_NAMES = [
  "arrowDown", "arrowLeft", "arrowRight", "arrowUp", "bell", "bellAlert", "bookmark", "building",
  "calendar", "calendarDays", "chart", "chartBar", "chartPie", "chatBubble", "check", "checkCircle",
  "chevronDown", "chevronLeft", "chevronRight", "chevronUp", "circleCheck", "clock", "close", "copy",
  "creditCard", "delete", "document", "documentText", "download", "edit", "fileLines", "filter",
  "globe", "grid", "heart", "history", "home", "info", "key", "lightBulb", "lightning", "list",
  "lock", "magnifyingGlassPlus", "mail", "map", "menu", "minus", "money", "paperclip", "play", 
  "plus", "presentationChartBar", "rocket", "search", "settings", "share", "shield", "signal", 
  "star", "swatch", "tableCells", "tag", "trendDown", "trendUp", "unlock", "upload", "user", 
  "userCircle", "userGroup", "view", "warning", "xCircle", "xMark", "photo"
];
    ...
    <div className="p-4">
      ...
      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-8 gap-4 mb-8">
        {SEMANTIC_ICON_NAMES.map(name => {
          ...
          const lightIconName = `${faIconName}Light`;
          
          return (
            <div 
              key={name}
              className="group flex flex-col items-center p-3 border rounded-md hover:bg-gray-50 transition-colors cursor-pointer"
            >
              <Icon 
                name={lightIconName}
                size="md"
      ...
      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-8 gap-4 mb-8">
        {KPI_ICON_NAMES.map(name => (
          ...
          >
            <img 
              src={KPI_ICON_URLS[name]} 
              alt={name}
              className="w-8 h-8 mb-2 transition-colors duration-200" 
              style={{ filter: 'invert(20%)' }} // Default dark color (Jet)
            />
      ...
      <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-8 gap-4 mb-8">
        {APP_ICON_NAMES.map(name => (
          ...
          >
            <img 
              src={APP_ICON_URLS[name]} 
              alt={name}
              className="w-8 h-8 mb-2 transition-colors duration-200" 
              style={{ filter: 'invert(20%)' }} // Default dark color
            />
            <span className="text-xs mt-1 text-center text-gray-600">{formatIconName(name)}</span>
          </div>
        ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-8">
        <div className="p-4 border rounded-md">
          ...
          <div className="flex justify-center space-x-4">
            {["user", "bell", "heart", "star"].map(name => (
              <div 
                key={name}
                className="group flex flex-col items-center"
              >
                <Icon 
                  name={`fa${name.charAt(0).toUpperCase() + name.slice(1)}Light`}
                  size="lg"
                  iconType="button"
                  action="default"
                />
        ...
        <div className="p-4 border rounded-md">
          ...
          <div className="flex justify-center space-x-4">
            {["trash-can", "xmark", "minus", "circle-xmark"].map(name => {
              const iconName = `fa${name.split('-').map((part, i) => 
      ...
      <div className="flex flex-wrap gap-8 mb-8">
        <div className="p-4 border rounded-md flex-grow">
          ...
          <div className="flex justify-center space-x-4">
            {["user", "bell", "heart", "gear", "check"].map(name => (
              <div 
                key={name}
        ...
        <div className="p-4 border rounded-md flex-grow">
          ...
          <div className="flex justify-center space-x-4">
            {["user", "bell", "heart", "gear", "check"].map(name => (
              ...
              >
                <Icon 
                  name={`fa${name.charAt(0).toUpperCase() + name.slice(1)}`}
                  size="lg"
                  iconType="static"
                  solid
                />
