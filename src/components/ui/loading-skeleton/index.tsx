...
export const SkeletonSection: React.FC<SkeletonSectionProps> = ({ 
  ...
  return (
    <div className={`bg-white rounded-lg shadow p-4 md:p-6 mb-4 md:mb-6 ${className}`}>
      {title && (
        <div className="flex justify-between items-start mb-4 md:mb-6">
          <div className={`h-5 md:h-6 bg-gray-200 rounded ${titleWidth} md:${titleWidth}`} />
          {actionButton && <div className="h-5 md:h-6 bg-gray-200 rounded w-10 md:w-16" />}
        </div>
      )}
      {lines > 0 && (
        <div className="space-y-3 md:space-y-4">
          {[...Array(lines)].map((_, i) => (
            <div key={i} className={`${lineHeight} bg-gray-200 rounded w-full`} />
          ))}
        </div>
      )}
      {children}
...
export const FormFieldSkeleton: React.FC<FormFieldSkeletonProps> = ({ 
  ...
  return (
    <div className="mb-4">
      {label && (
        <div className={`h-4 md:h-5 bg-gray-200 rounded ${labelWidth} mb-2`} />
      )}
      
      {type === 'text' && (
        <div className="h-8 md:h-10 bg-gray-200 rounded w-full" />
      )}
      
      {type === 'textarea' && (
        <div className="h-24 md:h-32 bg-gray-200 rounded w-full" />
      )}
      ...
      {type === 'upload' && (
        <div className="border-2 border-dashed border-gray-200 rounded-lg p-6 text-center">
          <div className="h-8 w-8 bg-gray-200 rounded-full mx-auto mb-3" />
...
export const WizardSkeleton: React.FC<{ 
  ...
  return (
    <div className={`w-full ${maxWidth} mx-auto p-4 md:p-5 animate-pulse`}>
      {/* Header Skeleton */}
      {hasHeader && (
        <div className="h-8 bg-gray-200 rounded w-full md:w-1/3 mb-6 md:mb-8" />
      )}
        ...
        <div className="mb-6 md:mb-8">
          ...
          <div className="flex justify-between">
            {[1, 2, 3, 4, 5].map((s) => (
              <div 
                key={s} 
                className={`h-6 bg-gray-200 rounded ${s === step ? 'w-14 md:w-24' : 'w-10 md:w-16'}`} 
              />
            ))}
          </div>
        </div>
      ...
      {stepContent ? (
        ...
        <>
            ...
            <div className="space-y-4 md:space-y-6">
              ...
              {step === 3 && (
                <>
                  <FormFieldSkeleton type="select" labelWidth="w-1/3 md:w-1/4" />
                  <div className="mb-6">
                    <div className="h-4 md:h-5 bg-gray-200 rounded w-1/3 md:w-1/4 mb-2" />
                    ...
                    {[...Array(2)].map((_, i) => (
                      <div key={i} className="border rounded-lg p-3 md:p-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                          <div className="h-32 md:h-40 bg-gray-200 rounded" />
                      ...
                      {[...Array(2)].map((_, i) => (
                        <div key={i} className="border rounded-lg p-3 md:p-4">
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="h-24 md:h-32 bg-gray-200 rounded" />
                            <div className="md:col-span-2 space-y-3">
                              {[...Array(3)].map((_, j) => (
          ...
          <div className="flex justify-between mt-6 md:mt-8">
            <div className="h-8 md:h-10 bg-gray-200 rounded w-20 md:w-24" />
            <div className="flex space-x-2 md:space-x-3">
              <div className="h-8 md:h-10 bg-gray-200 rounded w-24 md:w-32" />
              <div className="h-8 md:h-10 bg-gray-200 rounded w-20 md:w-24" />
            </div>
          </div>
        </>
    ...
    <div className="w-full animate-pulse">
      ...
      {hasFilter && (
        <div className="flex justify-between items-center mb-6">
          <div className="h-8 md:h-10 bg-gray-200 rounded w-64 md:w-80" />
          <div className="flex space-x-2 md:space-x-3">
            <div className="h-8 md:h-10 bg-gray-200 rounded w-24" />
            <div className="h-8 md:h-10 bg-gray-200 rounded w-24" />
          </div>
        </div>
      )}
      ...
      {hasHeader && (
        <div className="flex border-b pb-4 mb-4">
          {[...Array(cols)].map((_, i) => (
            <div 
              key={i} 
              className="h-6 bg-gray-200 rounded flex-1 mr-4 last:mr-0" 
              style={{ 
      ...
      {[...Array(rows)].map((_, i) => (
        <div key={i} className="flex py-4 border-b">
          {[...Array(cols)].map((_, j) => (
            <div 
              key={j} 
              className="h-5 bg-gray-200 rounded flex-1 mr-4 last:mr-0" 
              style={{ 
...
export const FormSkeleton: React.FC<{ 
  ...
  return (
    <div className="max-w-2xl animate-pulse">
      <div className={layout === 'grid' ? 'grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6' : 'space-y-4 md:space-y-6'}>
...
export const AuthSkeleton: React.FC<{
  ...
  return (
    <div className="animate-pulse">
      {showLogo && (
        <div className={`${logoSize} bg-gray-200 rounded-full mx-auto mb-6`} />
      )}
      ...
      {variant === 'signin' && (
        <div className="max-w-sm mx-auto">
          <div className="h-6 bg-gray-200 rounded w-40 mx-auto mb-8" />
          <div className="space-y-4">
            <FormFieldSkeleton type="text" />
            <FormFieldSkeleton type="text" />
            <div className="h-9 bg-gray-200 rounded w-full mt-6" />
          </div>
        </div>
      )}
...
const LoadingSkeleton: React.FC = CampaignDetailSkeleton;
