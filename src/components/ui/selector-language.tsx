...
export function LanguageSelector<TFieldValues extends FieldValues = FieldValues>({
  ...
  return (
    <Controller
      name={name}
      control={control}
      // Set RHF default based on allowMultiple, explicitly typing the array
      // @ts-ignore // TODO: Investigate defaultValue type mismatch for arrays
      defaultValue={allowMultiple ? ([] as string[]) : undefined}
                          ...
                          <Icon
                            iconId={
                              selectedValues.includes(language.value)
                                ? allowMultiple
                                  ? 'faCheckSolid'
                                  : 'faCircleDotLight' // Corrected to faCheckSolid
                                : 'faSquareLight'
                            }
                            className={cn(
                              'mr-2 h-4 w-4',
                              selectedValues.includes(language.value) ? 'opacity-100' : 'opacity-50'
                            )}
                            aria-hidden="true"
                          />
                          {language.label}
