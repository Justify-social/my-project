  ...
  if (type === 'svg') {
    return (
      <div className={containerClasses}>
        ...
        {showLabel && <p className={labelClasses}>{label}</p>}
      </div>
    );
  }
...
export function ButtonSpinner({ className }: { className?: string }) {
  return (
    <div className={cn('animate-spin rounded-full h-4 w-4 border-2 border-white border-t-transparent mr-2', className)} />
  );
}
...
export function InlineSpinner({ className }: { className?: string }) {
  return (
    <span className={cn('inline-block animate-spin rounded-full h-4 w-4 border-2 border-current border-t-transparent align-[-2px] mx-1', className)} />
  );
}
...
export function DotsSpinner() {
  return (
    <div className="flex space-x-1 justify-center items-center">
      ...
      <div className="h-2 w-2 bg-gray-600 rounded-full animate-bounce" style={{ animationDelay: '150ms' }}></div>
