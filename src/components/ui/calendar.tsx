...
export function Calendar({
  ...
  const renderDays = () => {
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
      days.push(
        <div key={`empty-${i}`} className="h-9 flex items-center justify-center text-gray-400 font-work-sans"></div>
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          ...
          {day}
          {dayEvents.length > 0 &&
          <span className="absolute bottom-1 left-1/2 transform -translate-x-1/2 w-1 h-1 rounded-full bg-[#3182CE] font-work-sans"></span>
          }
        </div>
      );
    }
  ...
  return (
    <div className={`${cn("bg-white overflow-hidden", className)} font-work-sans`}>
      <div className="flex items-center justify-between p-4 border-b font-work-sans">
        <h2 className="text-lg font-semibold font-sora">{MONTHS[month]} {year}</h2>
        <div className="flex space-x-2 font-work-sans">
          <button
            onClick={handlePrevMonth}
            className="p-1 rounded hover:bg-gray-100 font-work-sans"
      ...
      <div className="px-4 py-3 font-work-sans">
        <div className="grid grid-cols-7 gap-1 mb-2 font-work-sans">
          {DAYS_OF_WEEK.map((day) =>
          <div key={day} className="h-9 flex items-center justify-center font-medium text-sm text-gray-500 font-work-sans">
              {day}
            </div>
          )}
        </div>
