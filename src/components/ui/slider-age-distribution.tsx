...
import { FormLabel, FormItem, FormDescription } from '@/components/ui/form';
...
// Removed unused SUMMARY_COLORS constant
...
export function AgeDistributionSliderGroup<TFieldValues extends FieldValues>({
  ...
  description = 'Allocate percentages to each age range',
}: AgeDistributionSliderGroupProps<TFieldValues>) {
  const demographicsData = useWatch({ control, name });

  // Memoize current values using reduce for clean initialization
  const currentValues = useMemo(() => {
    return AGE_BRACKETS.reduce(
      (acc, bracket) => {
        // Ensure demographicsData is treated as potentially undefined record
        const currentDemoData = demographicsData as Record<string, unknown> | undefined;
        const value = currentDemoData?.[bracket.key];
        // Ensure value is parsed as number, default to 0
        acc[bracket.key] =
          typeof value === 'number' ? value : typeof value === 'string' ? parseFloat(value) : 0;
        if (isNaN(acc[bracket.key])) {
          acc[bracket.key] = 0; // Default to 0 if parsing failed
        }
        return acc;
      },
      {} as Record<AgeBracketKey, number>
    ); // Initialize accumulator with correct type
  }, [demographicsData]);

  const handleValueChange = useCallback(
    (changedBracketKey: AgeBracketKey, newValue: number) => {
      ...
      if (diff !== 0) {
        ...
        if (Math.abs(100 - roundedSum) > 0.1) {
          // If still significantly off, log error
      ...
      AGE_BRACKETS.forEach(bracket => {
        ...
        if (Math.abs(currentValues[bracket.key] - finalUpdates[bracket.key]) > 0.01) {
          setValue(
            `${name}.${bracket.key}` as FieldPath<TFieldValues>,
            finalUpdates[bracket.key] as PathValue<TFieldValues, FieldPath<TFieldValues>>,
            {
              shouldValidate: true,
              shouldDirty: true,
            }
  ...
  const finalSum = useMemo(
    () => AGE_BRACKETS.reduce((sum, b) => sum + (currentValues[b.key] || 0), 0),
    [currentValues]
  );
    ...
    <FormItem className="space-y-4">
      <div className="flex justify-between items-baseline mb-1">
        <div>
          ...
          {description && (
            <FormDescription className="mt-1 text-muted-foreground">{description}</FormDescription>
          )}{' '}
          {/* Use muted color */}
        </div>
        <span
      ...
      <div className="space-y-3 pr-1">
        {AGE_BRACKETS.map(bracket => (
          <div key={bracket.key} className="grid grid-cols-12 gap-x-2 items-center">
            <FormLabel className="col-span-2 text-sm text-muted-foreground text-right pr-1">
              {bracket.label}
            </FormLabel>
            ...
            <div className="col-span-2 flex items-center justify-end">
              <span className="text-sm font-medium w-8 text-right text-foreground tabular-nums">
                {currentValues[bracket.key].toFixed(0)}
              </span>
      ...
      <div className="mt-6">
        ...
        <div className="mt-2 flex justify-center">
          <div className="flex flex-wrap gap-2 justify-start items-center max-w-xl">
            {AGE_BRACKETS.map(bracket => {
              // Remove index dependency for color
