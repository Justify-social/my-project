...
export const List = forwardRef<HTMLDivElement, ListProps>(({
  ...
  return (
    <div
      ref={ref}
      className={cn(
        bordered && 'rounded-md border border-gray-200',
        ...
        {items.length > 0
          ? items.map((item, index) => (
              <ListItem
                key={index}
                {...item}
                divider={item.divider ?? (dividers && index < items.length - 1)}
                interactive={interactive}
                hoverable={hoverable}
                size={size}
                icon={item.icon || icon}
                layout={layout}
                highlighted={striped && index % 2 === 1}
              />
            ))
          : children}
      </ListTag>
    ...
    if (typeof icon === 'string') {
      return (
        <div className="mr-3 flex-shrink-0">
          <Icon name={icon as IconName} className="h-5 w-5 text-gray-400" />
        </div>
      );
    }
  ...
  return (
    <li
      ...
      className={cn(
        paddingClasses[size],
        layoutClasses[layout],
        divider && layout === 'vertical' && 'border-b border-gray-200 last:border-b-0',
        divider && layout === 'horizontal' && 'border-r border-gray-200 last:border-r-0',
      ...
      <div className="flex items-center">
        ...
        <div className="flex-grow min-w-0">
          {content ? (
            ...
            <>
              {primary && (
                <div className="text-gray-900 truncate">{primary}</div>
              )}
              {secondary && (
                <div className="text-gray-500 text-xs mt-0.5 truncate">{secondary}</div>
              )}
            </>
          )}
        </div>
...
export function ListExample() {
  ...
  const basicItems = [
    { primary: 'Inbox', icon: 'inbox' },
    { primary: 'Sent', icon: 'send' },
    { primary: 'Drafts', icon: 'file' },
    { primary: 'Trash', icon: 'trash' },
  ];
