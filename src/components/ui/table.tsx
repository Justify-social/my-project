...
// Internal components
const TableHeader = <T,>() => {
  const { columns, sortState, setSortState, compact, headerBgColor = 'white' } = useTableContext<T>();
  ...
  const handleSort = (column: ColumnDef<T>) => {
    ...
    setSortState({ column: direction ? id : undefined, direction });
  };
  ...
  return (
    <thead className={cn(headerBgColorClass.white, 'border-b border-gray-200')}>
      <tr>
        {columns.map((column) => {
...
const TablePagination = <T,>() => {
  const { data, pagination, currentPage, pageSize, setCurrentPage, setPageSize, compact } = useTableContext<T>();
