...
export function FileUploader<TFieldValues extends FieldValues = FieldValues>(
    ...
    const onDrop = useCallback((acceptedFiles: File[], fileRejections: any[]) => {
        ...
        if (fileRejections.length > 0) {
            ...
            else if (firstError.code === 'too-many-files') message = `Cannot select more than ${maxFiles} file(s)`;
                    ...
                    <FormControl>
                        ...
                        >
                            ...
                            {isUploading ? (
                                ...
                                <div className="text-center">
                                    <Icon iconId="faCircleExclamationLight" className="h-8 w-8 text-destructive mb-3" />
                                    <p id={`${name}-dropzone-error`} className="text-sm font-medium text-destructive">{errorState}</p>
                    ...
                    {localFiles.length > 0 && (
                        <div className="mt-3 space-y-2">
                            ...
                            {localFiles.map((file, index) => (
                                <div key={`${file.name}-${file.lastModified}`} className="flex items-center justify-between p-2 border rounded-md text-sm bg-muted/30">
                                    <span className="truncate flex-1 mr-2" title={file.name}>{file.name} ({(file.size / 1024).toFixed(1)} KB)</span>
                                    {!isUploading && (
                                        <Button
                                            type="button"
                                            variant="ghost"
                            ...
                            {!isUploading && (
                                ...
                                >
                                    <Icon iconId="faUploadLight" className="h-4 w-4 mr-2" /> Upload {localFiles.length} File(s)
                                </Button>
                            )}
                        </div>
                    )}
