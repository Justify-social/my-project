...
export function sanitizeApiPayload<T>(
  ...
  const { removeEmptyStrings = false, removeEmptyArrays = true, preserveFields = [] } = options;
    ...
    if (Array.isArray(value)) {
      if (value.length > 0 || !removeEmptyArrays) {
        ...
        result[key] = value.filter(item => {
          ...
          if (typeof item === 'object' && Object.keys(sanitizeApiPayload(item)).length === 0)
            return false;
          return true;
        });
      }
...
export function sanitizeContactFields<T extends Record<string, unknown>>(
  ...
  for (const fieldName of contactFields) {
    ...
    const hasCompleteData = contactObj.firstName && contactObj.surname && contactObj.email;
  ...
  if (sanitized.influencers && Array.isArray(sanitized.influencers)) {
    ...
    sanitized.influencers = sanitized.influencers.filter((influencer: unknown) => {
      ...
      if (
        ...
        Array.isArray(sanitized.influencers) &&
        sanitized.influencers.length === 1
      ) {
        return true;
      }
      ...
      return influencer && typeof influencer === 'object' && inf.platform && inf.handle;
    });
  }
 ...
 */
export function sanitizeStepPayload<T extends Record<string, unknown>>(
  payload: T,
  step: number
): Partial<T> {
