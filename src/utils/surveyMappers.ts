...
type ExtendedCampaign = CampaignWizardSubmission & {
  ...
  brandName?: string;
  musicTrack?: string;
  // Make startDate more flexible to handle different formats
  ...
  try {
    if (campaign.startDate) {
      ...
      if (typeof campaign.startDate === 'string') {
        ...
        try {
          const dateObj = new Date(campaign.startDate.toString());
...
function mapCreativeAsset(
  ...
  let primaryCreative = null;
  
  if (campaign.creativeAssets && campaign.creativeAssets.length > 0) {
    primaryCreative = campaign.creativeAssets[0];
  }
  ...
  return {
    id: primaryCreative.id || defaultCreative.id,
    type: primaryCreative.type === 'video' ? videoType : imageType,
    url: primaryCreative.url || defaultCreative.url,
    aspectRatio: primaryCreative.aspectRatio || "16:9", // Default aspect ratio
    thumbnailUrl: primaryCreative.thumbnailUrl || primaryCreative.url || defaultCreative.thumbnailUrl,
    duration: primaryCreative.type === 'video' ? (primaryCreative.duration || 15) : 0
  };
}
