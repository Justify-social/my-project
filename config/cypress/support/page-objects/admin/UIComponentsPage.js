...
export class UIComponentsPage extends BasePage {
  ...
  elements = {
    ...
    statusBadge: () =>
      cy
        .get(
          '.badge, [class*="bg-green-100"], [class*="bg-yellow-100"], [class*="bg-red-100"], [class*="bg-blue-100"]'
        )
        .first(),
  ...
  testInteractiveElements() {
    ...
    // Test all visible clickable elements only (exclude navigation/mobile elements)
    cy.get(
      'button:not([disabled]):visible:not([data-cy="mobile-menu-button"]):not([aria-label*="menu"]):not([aria-label*="navigation"])'
    ).each($button => {
      cy.wrap($button).should('be.visible').click({ force: false }); // Don't force click - test real accessibility
    });

    // Test keyboard navigation on visible buttons
  ...
  testAccessibility() {
    ...
    // Check for proper ARIA attributes (more permissive)
    ...
    // Test Tab navigation (using trigger keydown)
  ...
  testComponentStates() {
    ...
    // Test normal state (find visible buttons only)
    cy.get('button:not([disabled]):visible')
      .first()
      .should('be.visible')
      .and('not.have.attr', 'disabled');

    // Test hover state (trigger hover)
    cy.get('button:not([disabled]):visible').first().trigger('mouseover').should('be.visible');

    // Test focus state
    cy.get('button:not([disabled]):visible').first().focus().should('have.focus');

    // Test disabled state (if any exist)
    cy.get('body').then($body => {
      if ($body.find('button[disabled]').length > 0) {
        cy.get('button[disabled]').first().should('be.disabled');
      }
    });
