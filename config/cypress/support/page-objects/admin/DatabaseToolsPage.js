...
export class DatabaseToolsPage extends BasePage {
    ...
    elements = {
        ...
        tableExpander: (tableName) => this.elements.tableItem(tableName).find('[data-testid="expand-table"]'),
        columnsList: (tableName) => this.elements.tableItem(tableName).find('[data-testid="columns-list"]'),
        columnItem: (tableName, columnName) => cy.get(`[data-testid="column-${tableName}-${columnName}"]`),
    ...
    expectTableRowCount(expectedCount) {
        this.elements.tableDataGrid().find('[data-testid^="table-row-"]').should('have.length', expectedCount);
        return this;
    }
    ...
    browseTableData() {
        ...
        cy.get('[data-testid="table-selector"] option').eq(1).then($option => {
    ...
    validateDatabaseMetrics(expectedMetrics) {
        ...
        if (expectedMetrics.minRecords !== undefined) {
            this.elements.totalRecords().invoke('text').then(text => {
