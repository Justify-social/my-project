...
export class BillingPage extends BasePage {
  ...
  elements = {
    ...
    selectPlanButton: planName =>
      this.elements.pricingCard(planName).find('button').contains('Select'),
    upgradePlanButton: planName =>
      this.elements.pricingCard(planName).find('button').contains('Upgrade'),
    currentPlanBadge: () => cy.contains('.badge', 'Current Plan'),

    // Plan features
    planFeaturesList: planName =>
      this.elements.pricingCard(planName).find('[data-cy="plan-features"]'),
