...
export class TeamManagementPage extends BasePage {
    ...
    elements = {
        ...
        removeMemberButton: (memberEmail) => this.memberItem(memberEmail).find('button').contains('Remove'),
        ...
        resendInviteButton: (email) => this.pendingInvitationItem(email).find('button').contains('Resend'),
        revokeInviteButton: (email) => this.pendingInvitationItem(email).find('button').contains('Revoke'),
    ...
    expectMemberCount(expectedCount) {
        this.elements.membersList().find('[data-testid="member-item"]').should('have.length', expectedCount);
        return this;
    }
