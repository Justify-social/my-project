...
export class ProgressPage extends BasePage {
    ...
    elements = {
        ...
        targetValue: () => this.elements.progressCounter().contains('/').then($el => $el.text().split('/')[1]),
        currentValue: () => this.elements.progressCounter().contains('/').then($el => $el.text().split('/')[0]),
    ...
    expectProgressData(expectedData) {
        if (expectedData.current !== undefined && expectedData.target !== undefined) {
            this.elements.progressCounter().should('contain', `${expectedData.current} / ${expectedData.target}`);
        }

        if (expectedData.percentage !== undefined) {
            this.elements.progressBar().should('have.attr', 'aria-valuenow', expectedData.percentage.toString());
        }
    ...
    testRealTimeUpdates() {
        ...
        this.elements.progressCounter().then($counter => {
            const [current, target] = $counter.text().split(' / ').map(n => parseInt(n));
            ...
            this.elements.progressCounter().then($newCounter => {
                const [newCurrent, newTarget] = $newCounter.text().split(' / ').map(n => parseInt(n));
    ...
    validateProgressMetrics(expectedMetrics) {
        ...
        if (expectedMetrics.responseCount !== undefined) {
            this.expectResponseCounts(expectedMetrics.responseCount.current, expectedMetrics.responseCount.target);
        }
