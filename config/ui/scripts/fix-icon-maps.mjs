...
function kebabCase(str) {
  return str.replace(/^[A-Z]/, g => g.toLowerCase()).replace(/[A-Z]/g, g => `-${g.toLowerCase()}`);
}
...
function generateIconRegistry(svgFiles) {
  ...
  ICON_CATEGORIES.forEach(category => {
    ...
    categoryFiles.forEach(fileData => {
      ...
      const displayName = id.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
...
function saveRegistry(registry) {
  if (DRY_RUN) {
    console.log(
      `${colors.yellow}[DRY RUN] Would save registry with ${registry.icons.length} icons${colors.reset}`
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `${colors.green}✅ Successfully saved icon registry with ${registry.icons.length} icons${colors.reset}`
    );
  } catch (error) {
...
function main() {
  ...
  console.log(
    `${colors.green}Generated icon registry with ${iconRegistry.icons.length} entries:${colors.reset}`
  );
  ...
  console.log(
    `${colors.green}${iconsWithMap.length} icons have the 'map' property:${colors.reset}`
  );
