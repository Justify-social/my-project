...
mcp_Graphiti_add_episode({
  ...
  episode_body: "Tooltips should appear above elements by default, falling back to below only when space is constrained.",
  source: "text",
...
mcp_Graphiti_add_episode({
  ...
  episode_body: "When integrating with ExampleAPI, implement exponential backoff for rate limit errors starting with 500ms delay.",
  source: "text",
